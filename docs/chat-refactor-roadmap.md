...
**146 bugs · 101 sequenced into 6 phases · 41 explicitly outside · 0 UNSEQUENCED · 32 of the 101 sequenced have NO OWNER**
...
| ☐ | `integrate` | — | OWNER(appeals seat): FIXING THE ROUTING FIRST WOULD HAVE MADE THE SYMPTOM WORSE, NOT NEUTRAL -- THE MISROUTE W |
| ☐ | `integrate` | chat | HOW STEP 2 OF THE FIX MUST ACTUALLY BE WRITTEN -- Chat Master found the durable read and I verified it, then f |
| ☐ | `integrate` | chat | CHAT IS IN THE EXACT STATE THAT ALREADY CAUSED A 29-DAY SILENT OUTAGE ON THIS FLEET, AND I HAD THAT PRECEDENT  |
| ☐ | `integrate` | — | OWNER(chat + appeals seat): THE TWO ENDS OF ONE WIRE ARE A MAJOR VERSION APART ON THE SAME PROTOCOL LIBRARY, A |
| ☐ | `integrate` | — | OWNER(chat + platform): FOUR SEATS SPENT THE DAY VERIFYING A COMMIT AND NONE OF US VERIFIED THE IMAGE |
...
| ☐ | `integrate` | chat | THREE APPEALS TOOLS ARE DISPATCHED TO A SERVER THAT DOES NOT HAVE THEM, AND THE ERROR IS CITED AS A SOURCE |
...
| ☐ | `tool_manifest` | chat | FOUND IT — carc_group SILENTLY OVERRIDES A CORRECT CARC CODE, AND GROUP MATCHING IS CASE-SENSITIVE |
...
## P2 — Make absent producers detectable  ·  37 items
...
| ☐ | `integrate` | — | OWNER(platform, method): THE DISQUALIFYING FACT WAS FOUR CHARACTERS TO THE LEFT -- A SELF-LABELLED PRINT ASSER |
| ☐ | `integrate` | chat | THE BADGE CAN SAY 'no sources' WITHOUT EVER LOOKING AT THE SOURCES, AND ITS OWN INPUTS ARE NOT RECORDED |
| ☐ | `integrate` | chat | THE SOURCE BADGE IS COMPUTED WRONG ON EVERY MCP TURN, AND THE MODEL'S OPINION IS WHAT RESCUES IT |
...
| ☐ | `state_load` | — | OWNER(chat, self-reported): state_load:blocks_assembled IS SINGLE-VALUED — THE COUNT TARGETS A CONSTANT AND TH |
