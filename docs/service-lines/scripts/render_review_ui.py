...
.note{padding:var(--mobius-space-base) var(--mobius-space-md);
  ...
  .srccard{border-color:color-mix(in srgb,var(--mobius-accent) 35%,var(--mobius-border))}
  .srcbar{display:flex;align-items:center;gap:6px;flex:0 0 auto}
  .srccard .throw{gap:var(--mobius-space-md);flex-wrap:wrap}
...
var CITE = {verbatim:["In the document","c-ok"],
            /* A fee schedule states its facts in table columns, so a claim resolving to
               a table row is the PRIMARY source — better evidenced than a sentence about
               it elsewhere, not worse. Ranked with verbatim. */
            in_table:["In the fee schedule table","c-ok"],
            composed:["Summarised from the document","c-ok"],
...
function draw(){
  ...
  /* Starting a run is the PRIMARY action for a service that still has unsourced
     requirements, so it sits here — above the fold, nothing to expand. It was inside
     Technical details, collapsed, below 2,000px of cards; the button rendered but
     offsetParent was null, which is a control that exists and cannot be found.
     The step HISTORY stays in Technical details. Doing the work and reviewing what the
     work did are different jobs. */
  h += '<section class="card srccard">'+
    '<div class="pad">'+
      '<div class="throw">'+
        '<div><strong>'+(l.toSource
            ? l.toSource+' requirement'+(l.toSource===1?'':'s')+' with no source yet'
            : 'Every requirement here has a source')+'</strong>'+
          '<div class="hint2">'+(l.toSource
            ? 'Ask the policy documents for the missing answers. Every step is recorded.'
            : 'You can still re-ask to check the answers against the documents again.')+
          '</div></div>'+
        '<span class="srcbar">'+
          '<label class="mini" for="srcprof">Answer using</label>'+
          '<select class="sel" id="srcprof"></select>'+
          '<button class="btn-src btn pri" data-src="'+esc(l.id)+'">Source this service</button>'+
        '</span>'+
      '</div>'+
  ...
  var tech =
    ...
    '<div class="pad" style="border-top:1px solid var(--mobius-border)">'+
      '<h3 class="th">Sourcing history</h3>'+
      (l.runs.length ? l.runs.map(runBlock).join("")
        : '<p class="hint2">No runs yet. Use <em>Source this service</em> at the top of '+
          'this page; every step will be recorded here.</p>')+
    '</div>'+
    '<div class="pad" style="border-top:1px solid var(--mobius-border)">'+
