...
- **Panel Credentialing Audit Report** — waterfall structure (Section A guaranteed, B at-risk, C missing, D taxonomy-opportunity), executive summary, issues across the panel, per-provider profiles; exportable as PDF.
...
- **Postgres (skill-owned tables)** — `provider_roster` master (FK to `org_profile`), plus `provider_nppes_snapshot`, `provider_pml_snapshot`, `provider_compliance_check`, org/NPI, briefing-module, and NPPES-provenance tables (migrations 015–029).
