...
- Per-slide **chat drill-down** (💬 button or press `A`): every one of the 31 slides has a `system_prompt.md`, so chat is live on all of them. The deck posts active-slide context; round 0 answers from context, else returns `NEEDS_TOOLS` and falls through to the full ReAct tool loop. Slide chat commonly uses `search_corpus`, `get_org_profile`, `get_org_benchmark`, `get_rate_benchmarks`, `get_published_rates`, `get_market_timeseries`, `get_entrant_analysis`, `get_fact_pack`. (2026-07-02: the `admin-s2b-orbital-v2` / `admin-s2b-score` prompts were fixed to route G2/Capterra evidence questions to `search_corpus` instead of answering "cannot answer.")
- **PDF export** (⎙ PDF button, top-right; live 2026-07-02): `window.print()` with `@media print` CSS (landscape, one slide per page, chrome hidden, dark slides preserved via `print-color-adjust:exact`) and a `beforeprint` handler that re-renders the JS-drawn score table + Chart.js canvases. **Known gap:** the `admin-overview`/`admin-s1`/`admin-s1c` charts render blank in the PDF unless the user has navigated to those slides first (their data loads on slide visit) — click through all slides before printing.
...
- **Acts VI–VIII (business / investment section).** 16 slides built in `public/acts_preview.html` (Act VI The Engagement, Act VII Capital Required incl. the interactive worksheet, Act VIII The Commitment) but **not yet merged into `story.html`** — a standalone preview, not part of the live deck.
...
- **Refreshed 2026-07-03** from the Strategy agent's inventory: added PDF export, per-slide chat tool list + the NEEDS_TOOLS routing fix, and the Acts VI–VIII preview; confirmed 31 slides served from the `story.html` monolith (~8,400 lines) and the live deploy at `mobius-story-ui-ortabkknqa-uc.a.run.app`.
