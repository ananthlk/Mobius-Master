...
.dep{display:inline-block;font-size:var(--mobius-text-xs);padding:0.15rem 0.5rem;
 border-radius:var(--mobius-radius-full);background:var(--mobius-bg-tertiary);
 margin-left:var(--mobius-space-sm);font-family:var(--mobius-font-mono)}
.dep-ok{color:var(--mobius-success)}
.dep-behind{color:var(--mobius-warning)}
...
.c-pending{color:var(--mobius-warning)}
...
// DEPLOY BANNER — what is RUNNING, beside what is written. The page used to describe
// committed code and let the reader assume it was live.
(function(){
  var dp = D.deploy || {};
  var el = document.getElementById('lead');
  if (!el || !dp.revision) return;
  var behind = parseInt(dp.commits_behind_head || '0', 10);
  var ok = (behind === 0) && (String(dp.traffic_percent) === '100');
  var span = document.createElement('span');
  span.className = 'dep ' + (ok ? 'dep-ok' : 'dep-behind');
  span.textContent = dp.checked === false
    ? 'deploy state UNVERIFIED (last known ' + dp.revision + ')'
    : 'LIVE ' + dp.revision + ' @ ' + dp.traffic_percent + '% · commit ' +
      (dp.commit||'?') + (behind ? ' · ' + behind + ' COMMITS BEHIND HEAD' : ' · up to date');
  span.title = dp.checked === false
    ? 'gcloud was unavailable. Could-not-check is not the same as not-deployed.'
    : 'Deployed image commit. A finding stamped FIXED at a commit not in this image '
      + 'renders "fixed · NOT DEPLOYED".';
  el.appendChild(span);
})();
...
function detail(k){
  ...
  var fitems = (m.findings||[]).map(function(f, i){
      var st = fst[i] || {};
      var cls = 'f-'+f[0], chip = '';
      if (f[0] === 'bad' && st.closed) {
        cls = 'f-done';
        // A closure stamp says the code CHANGED. Whether it is RUNNING is a separate
        // fact, and conflating them is how I told Ananth a live bug was fixed and a
        // fixed bug was live, on the same day.
        chip = st.deploy === 'not_deployed'
          ? '<span class="fchip c-pending">fixed · NOT DEPLOYED</span>'
          : (st.deploy === 'live'
              ? '<span class="fchip c-done">closed · live</span>'
              : '<span class="fchip c-done">closed</span>');
      }
