...
REGION = "us-central1"
NO_OPENAPI_PROBES = ["/", "/docs", "/index.html", "/health", "/admin", "/ui"]
TIMEOUT = 25
...
def openapi_paths(url: str, tok: str) -> list[str] | None:
    try:
        ...
        return None
    return sorted(p for p, ops in d.get("paths", {}).items()
...
def main() -> int:
    ...
    for s in svcs:
        ...
        if paths is None:
            ...
            for p in paths:
                jobs.append((s["service"], s["url"], p, "openapi"))

    def run(j):
        svc, base, path, how = j
        u = base.rstrip("/") + ("" if path == "/" else path)
        st, ct = probe(u)
        return {"service": svc, "path": path, "url": u, "http": st,
    ...
    def second(r):
        if r["kind"] == "auth-required":
            ...
            r["kind"] = ("surface-authed" if k == "surface"
                         else ("api-authed" if k == "api" else r["kind"]))
    ...
    surfaces = [r for r in rows if r["kind"] in ("surface", "surface-authed")]
