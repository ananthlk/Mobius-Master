...
""", findings=[
 ...
 ("bad", "OWNER(chat): THE TEST SUITE MUTATES THE BANDIT'S LIVE STATE — TO BE TESTED IN "
         ...
         "asserts on.\n\n"
         "THE BROKEN TEST IS THE SMALL HALF. The counters the suite moves are the SAME ONES "
         "THAT DRIVE THE BANDIT'S PHASE TRANSITIONS: Phase 1->2 at 10 quality samples, Phase "
         "2->3 (locked exploitation) at 100. A test run that adds 37 samples to one model is "
         "pushing it toward exploitation on evidence that is not traffic. Whether that leaks "
         "beyond the process is the open question — the in-process roster does not persist, "
         "but :2317-2323 reads FROM Postgres, so the direction of contamination needs "
