...
""", findings=[
 ...
 ("good", "SETTLED BY READING THE APPEALS IMAGE -- the anyio-40 figure Appeals said could not "
          ...
          "mobius-appeals-prototype:pintest-20260908: the image carries anyio 4.15.1, starlette "
          "1.6.0, fastapi 0.141.1, httpx 0.28.1, uvicorn 0.52.4, mcp 1.30.0. I then installed "
          "anyio==4.15.1 in a scratch venv: default thread limiter total_tokens == 40. So the "
 ...
 ("bad", "OWNER(chat): THE LAST LINE OF DEFENCE STRING-MATCHES AN ERROR MESSAGE IT DOES "
          ...
          "RETRACTED AS AN EXPLANATION, KEPT AS AN INDEPENDENT LATENT DEFECT. I paired this with the getattr fail-open and claimed a complete path to success=True with no bug anywhere. THE FIRST LEG IS NOW EXCLUDED, not merely unconfirmed: the primary sends isError EXPLICITLY True (in model_fields_set), and Chat Master then checked the pin FLOOR -- mcp 1.0.0, 1.2.0 and 1.26.0 ALL carry the isError field, and requirements.txt:113 allows mcp>=1.0.0, so EVERY version the image could legally contain parses True -> (text, False) -> failure branch -> no sources. No SDK in range can produce the observation. Appeals, who built the stacked story on my lead, has withdrawn their half too. So :2792 is a real off-by-one-word defect on the same path, not half an explanation. The disproven chain was: (1) deployed SDK renames or "
