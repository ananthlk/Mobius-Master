...
""", findings=[
 ...
 ("good", "FIXED (chat, 185f426, 2026-09-09): the wrapper no longer collapses every "
          "failure into one verdict. A publish failure now reports "
          "'blocked_publish_failed' and carries the classifier's REAL verdict; "
          "fail-closed is untouched — a publish failure still blocks. Two causes in "
          "this area belong to other seats and are routed, not fixed here: rag's "
          "non-idempotent publish (9 occurrences/24h, PRE-DATES the chat deploy, so it "
          "is not a regression we introduced), and phi_classify tripping circuit "
          "breakers at 11.6s against a 3.4s ema. From this deploy on, a classifier "
          "timeout is attributable in the log instead of hidden inside a generic "
          "indeterminate."),
 ("bad", "OWNER(chat): THERE IS NO SUCH THING AS 'THE PHI GATE TIMEOUT'. THREE CALL "
         "SITES, THREE DIFFERENT VALUES, ONE OF THEM NOT CONFIGURABLE AT ALL — and I "
         "conflated them when reporting to the phi-classifier seat, which nearly "
         "produced a wrong change. (1) UPLOAD gate, app/main.py:1307, POST /classify, "
         "timeout=45 HARDCODED. (2) MESSAGE gate, app/api/chat.py:247, "
         "POST /message-check, timeout=4.0 HARDCODED — no env var, cannot be tuned "
         "without a deploy, and it sits on the SYNCHRONOUS path before the user sees "
         "any acknowledgement. (3) FEEDBACK gate, app/skills/phi_gate.py:30, "
         "PHI_GATE_TIMEOUT_SEC default 4 — the only configurable one, and the least "
         "important. A single env var name that governs only the path nobody asked "
         "about is worse than none: it reads as the system's timeout knob and is not. "
         "VERIFIED 2026-09-09, and it is worse than 'misleading': PHI_GATE_TIMEOUT_SEC "
         "is read at exactly ONE site (phi_gate.py:30) and that module is imported by "
         "exactly TWO callers — app/api/product_feedback.py:111 and "
         "app/skills/builtin/product_feedback.py:259. It governs PRODUCT FEEDBACK TEXT "
         "AND NOTHING ELSE. Setting PHI_GATE_TIMEOUT_SEC=8 is a NO-OP for document "
         "uploads (45s hardcoded) and for chat messages (4.0 hardcoded). The "
         "phi-classifier seat recommended exactly that setting twice, reasoning about "
         "document latency, in good faith and off a name I gave them. Someone would "
         "have set it, watched nothing change, and drawn a conclusion about the "
         "classifier. A knob that cannot move the thing it appears to name is a "
         "CONFIGURATION-SHAPED PLACEBO — same family as the health signal that is "
         "green by construction. RESOLVED ON VALUE, 2026-09-09: the phi-classifier "
         "seat re-measured /message-check on the RIGHT path — p50 332ms, p95 561ms, "
         "max 561ms over 15 warm chat-shaped calls, 7x headroom under the 4s — and "
         "WITHDREW the 8s recommendation. /message-check is regex+NER only, no LLM by "
 ...
 ("bad", "OWNER(fleet-power seat): A HIPAA GATE'S CORRECTNESS DEPENDS ON A SCALING "
         ...
         "15s on a cold start — so a message arriving at a cold instance times out and "
         "FALSE-BLOCKS. Today that cannot happen, because fleet.yaml:45 pins "
         "mobius-phi-classifier at min:1. The problem is WHY it is pinned, as recorded: "
         "'HIPAA gate; throttled CPU so idle min is cheap' — that is a cost "
         "justification, and it invites a future cost sweep to reasonably reclaim it. "
         "Compare fleet.yaml:50, where the web-scraper carries 'min=1 REQUIRED: brpop "
         "poll loop — min=0 stops queue polling and jobs sit pending forever (the "
         "2026-08-17 outage)'. That note cannot be misread; this one can. The "
         "invariant — min:1 is a CORRECTNESS requirement of the chat PHI gate, not a "
         "cost preference — exists only in a session transcript. Raised by the "
         "phi-classifier seat 2026-09-09 while withdrawing an unrelated "
         "recommendation; they were explicit they would rather hold min-instances than "
         "widen the message timeout, and they are right. FIX: one line in fleet.yaml "
         "stating the dependency and naming the consequence. "
         "CLOSED 2026-09-09 by the fleet-power seat, fleet.yaml c3688b7, VERIFIED "
         "FIRSTHAND HERE rather than accepted on report. The stanza now carries a "
         "REQUIRED-comment block in the crawler's style: 'min=1 REQUIRED (correctness, "
         "not cost): chat's message PHI gate calls /message-check with a 4s timeout "
         "(mobius-chat app/api/chat.py:247) and is fail-closed; a cold start loads "
         "Presidio/spaCy in ~15s, so min=0 turns every cold start into a FALSE-BLOCK "
         "of a legitimate user message. Idle min being cheap (~$39/mo at the idle SKU) "
         "is the secondary point.' Cost demoted to secondary, apply --dry-run clean, "
         "no functional change. They also verified the 4s at chat's own call site "
         "before writing it in rather than taking my citation — which is the right "
         "instinct given that a wrong line number from me is exactly what nearly "
         "shipped an 8s message gate earlier the same day."),
 ("good", "PHI CLASSIFIER SEAT SHIPPED A REAL LATENCY FIX, 2026-09-09 "
          "(mobius-phi-classifier-00029-ns4, live): the classifier now SKIPS the LLM "
          "contextual pass when regex/NER already found strong PHI, because a strong "
          "identifier blocks regardless and the LLM can only clear WEAK-only hits — so "
          "the 3-5s Vertex call was pure latency with no effect on the verdict. "
          "Caller-scoped: /redact still runs the LLM, /message-check was regex+NER "
          "already. Measured warm: explicit-PHI doc 0.28-0.32s (was ~4-5s, ~15x); "
          "clean doc 2.0s small / 5.66s at 14KB, where the LLM is necessary. THE "
          "STRUCTURAL WIN, which is better than the speedup: explicit-PHI documents are "
          "now removed from the timeout's exposure ENTIRELY — they return in ~0.3s and "
          "can never time out into a false indeterminate. Whatever the timeout is, it "
 ...
 ("watch", "PHI-CLASSIFIER SEAT RULED 2026-09-09 on the fail-open item: REFUSE. An "
           ...
           "strongest case to block, not the exception allowed through. Must emit an "
           "attributable verdict (action='blocked_unconfigured'), never a silent skip. "
           "Chat implements. On latency they ruled 3.6-4.2s is EXPECTED not a "
           "regression — it is the LLM contextual pass on the ingestion critical path, "
           "a deliberate P0 choice, with regex+NER sub-second; the 11.6s tail is Vertex "
           "tail latency. They recommend 8s and will cut their p50 under ~3s. CAVEAT "
           "MINE, and it changes the recommendation: their 5.66s measurement was a 14KB "
           "DOCUMENT through the UPLOAD path, whose timeout is already 45s and was "
           "never 4s. The 4s they were warned about is the MESSAGE gate on a chat "
           "message — a different and much smaller workload. Their number does not "
           "size that budget, and neither does mine."),
 ("bad", "OWNER(phi-classifier seat): THE GATE FAILS OPEN ON MISCONFIGURATION. If "
         "PHI_CLASSIFIER_URL is unset the upload gate is SKIPPED ENTIRELY and the "
         "document publishes — no classification, no block, no audit row. Not live "
         "exposure: it IS set on dev and everywhere we deploy. But it is the single "
         "input that lets a document through unscreened, in a path that is otherwise "
         "deliberately fail-closed on every other failure (timeout, non-200, "
...
""", findings=[
 ...
 ("bad", "OWNER(chat, self-reported): state_load:blocks_assembled IS SINGLE-VALUED — "
         "THE COUNT TARGETS A CONSTANT AND THE DETAIL LIVES ONLY IN THE LOG. Chat "
         "Master 2026-09-10, caught by their own is_fallback rule applied to their own "
         "instrument. The count target is the literal string 'blocks_assembled', while "
         "the per-block present/empty/absent/not_run/read_failed detail is in the LOG "
         "LINE. So the countable signal carries no information — it is is_fallback "
         "exactly, in a field added hours after they wrote the rule for me, and the "
         "second time that has happened. The log is fine; the queryable signal is not. "
         "Their proposed fix: target the count on the block PATTERN rather than a fixed "
         "label, so grouping shows which combinations actually occur. FOUR OTHER "
         "SIGNALS ARE ALSO SINGLE-VALUED AND ARE NOT DEFECTS — react_retry_guard "
         "'allowed' 19 / never blocked, retrieval_budget 'computed' 2 / floor never "
         "bound, active_context 'none_available' 10, feedback_signal 'skipped' 3. That "
         "is honest low traffic, and reporting what APPEARED rather than what COULD "
         "appear is exactly the instruction; the distinction between a single-valued "
         "field and a single-valued SAMPLE is the whole reason the rule needs judgement "
 ...
 ("bad", "OWNER(chat): ROOT CAUSE OF THE PER-READ OVERHEAD — EVERY POOLED ACQUIRE RUNS "
         ...
         "CORRECTION, and it is the FOURTH instance this session of the same trap: I relayed "
         "'starves a max=10 pool'. The live value is CHAT_DB_POOL_MAX=20; TEN IS THE SOURCE "
         "DEFAULT at db_client.py:227. Eval read the default, I passed it on without "
         "checking, Chat Master read the deployed environment. It does not touch the "
         "three-round-trips finding but it halves the starvation math, which matters to "
         "anyone sizing the pool. Same shape as the governor reading 'off' while deployed "
         "true, the integrator 'sequential' while parallel, and enrichment '0%' while 100 — "
         "and it keeps recurring for the reason already logged on this node: 116 of 187 env "
         "vars run on invisible code defaults and no surface states the effective value.\n\n"
         ...
         "connection and starves the pool. That is the same cleanup-on-failure shape as "
         ...
         "the wall said otherwise. Chat Master's generalisation, kept in their words: ANY "
         "PER-TARGET COUNTER IS BLIND TO WHATEVER HAPPENS BETWEEN DECIDING TO CALL AND THE "
         "CALL STARTING.\n\n"
         "MEASURED 2026-09-09 ON THE DEPLOYED INSTRUMENT — MECHANISM CONFIRMED, FIX "
         "DECLINED. Chat Master measured; I verified from turn_spans independently.\n\n"
         "  acquires == db ops EXACTLY, per turn: 7 acquires for 5 reads + 2 writes; 5 for 5.\n"
         "  So the per-query acquire predicted from CHAT_DB_MODE=direct is real.\n"
         "  acquire = 75-115ms per turn, 10.7-16.4ms each, 38-71% OF ALL DB TIME.\n\n"
         "AND THE SHARE THAT DECIDES IT — my own numbers, four turns:\n"
         "    wall 52,366ms  db 266ms (0.51%)  acquire 111ms (0.21%)\n"
         "    wall 47,666ms  db 272ms (0.57%)  acquire  75ms (0.16%)\n"
         "    wall 46,297ms  db 347ms (0.75%)  acquire 115ms (0.25%)\n"
         "    wall  9,639ms  db 221ms (2.30%)  acquire  93ms (0.97%)\n\n"
         "So eliminating connection acquire ENTIRELY buys roughly 0.2% of turn wall on an "
         "agentic turn. The connection-per-turn refactor's second half is guaranteed release "
         "on every exit path including the 20s LLM timeout — the same cleanup-on-failure "
         "shape as the state_load bug this node just fixed. THAT RISK FOR 0.2% IS NOT WORTH "
         "SPENDING, and Chat Master said so rather than letting a confirmed diagnosis become "
         "a work item by momentum. Recorded as a decision, not a deferral.\n\n"
         "WHAT REMAINS UNSETTLED, and it is the honest limit: these are WARM turns — reads at "
         "6-17ms. They do NOT reproduce the 260-630ms I measured earlier, which was cold-pool "
         "or contended and consistent with the PoolError 50ms-retry path. So the mechanism is "
         "confirmed and its MAGNITUDE UNDER CONTENTION IS UNQUANTIFIED. A per-turn connection "
         "would help a cold or contended turn far more than a warm one. n=4 warm turns cannot "
         "settle that, and nobody should size the win from these numbers.\n\n"
         "MY OWN FIRST READING WAS WRONG: I reported 2 acquires against 15 reads and called "
         "the three-round-trips theory into question. That sample straddled the deploy and "
         "mixed in old-revision spans carrying no acquire instrument. Chat Master's numbers "
         "were right and mine were incomplete.\n\n"
         "AND THE NODE'S OWN SHAPE CHANGED: state_load processing ran 80-327ms across those "
         "turns — now LARGER than its DB time including acquire. The 'four sequential slow "
         "reads' framing this node started with no longer describes it.\n\n"
         "NOT FIXED, DELIBERATELY, and the reasoning is the right one: the arithmetic is a "
         "floor built on an unmeasured base RTT, and the second half of the fix is a "
         "connection-LIFECYCLE change — the same cleanup-on-failure shape as the state_load "
         "bug. Chat Master built the INSTRUMENT instead (f2db529): db.acquire recorded at all "
         "three acquire sites and reported alongside db_read_ms/db_write_ms, never summed "
 ...
 ("bad", "OWNER(chat): state_load IS 1.2s AT p50 AND IT IS ALL SEQUENTIAL DB READS — "
         ...
         "MEASURED AFTER, 2026-09-10 on 00977-8bl: BEFORE n=217 p50 1572.9ms p95 5558.0ms; AFTER n=9 p50 412.7ms p95 687.9ms — p50 DOWN 74%. turn_spans, module='state_load', label IS NULL, split on the revision-ready timestamp 14:10:30Z. CAVEATS ARE CHAT MASTER'S OWN AND I AM KEEPING THEM RATHER THAN QUOTING THE HEADLINE: read the p50, not the p95 — at n=9 a p95 is the second-highest sample, not a tail. And the before window is 11 hours of mixed traffic INCLUDING COLD STARTS against nine warm turns, so it is NOT LIKE-FOR-LIKE ON TRAFFIC MIX; the steady-state p50 should be expected to land above 412ms once cold instances enter the sample. THE DIRECTION AND ROUGH MAGNITUDE ARE SOLID; THE EXACT FIGURES ARE NOT YET. A seat that volunteers the weakness in its own headline number is the reason this one can be used at all."),
...
""", findings=[
 ("bad", "OWNER(appeals seat): FIXING THE ROUTING FIRST WOULD HAVE MADE THE SYMPTOM "
          "WORSE, NOT NEUTRAL -- THE MISROUTE WAS SUPPRESSING THE LOAD THAT TRIGGERS THE REAL "
          "BUG. Appeals found the mechanism behind the CARC 197 timeout and it inverts the fix "
          "order. Their measurement: /rules/197 answers in 0.149s / 0.224s (8,997 bytes), "
          "/rules/29 0.148s, /rules/22 0.154s, /rules/50 0.140s -- while _get() has a 30-SECOND "
          "timeout. A 30s timeout fired against an endpoint answering in 0.15s, so the backend "
          "is not slow. Their cause: api/mcp_server.py:57 _base() is a SELF-URL "
          "('appeals agent calls its own REST endpoints') and the MCP server is mounted in the "
          "SAME FastAPI app (api/app.py:37). So an MCP tool call occupies a request worker and "
          "then makes a synchronous httpx call back into its own process, blocking on a worker "
          "it is itself holding. WHAT I VERIFIED INDEPENDENTLY against the deployed service "
          "(their repo is not checked out here, so their file:line claims are their reading, "
          "not my measurement): maxScale='2', minScale ABSENT on both template and service "
          "(scale-to-zero), APPEALS_AGENT_SELF_URL is NOT SET -- so _base() falls back to "
          "http://localhost:8080 exactly as they describe -- and fleet.yaml:89 has "
          "mobius-appeals-prototype {mode: standby}. ONE CORRECTION TO THEIR ACCOUNT, offered "
          "because it changes the failure profile rather than the conclusion: "
          "containerConcurrency is 80, not 1, so this is NOT a guaranteed deadlock. A single "
          "instance can serve its own callback. The failure is THREAD-POOL STARVATION under "
          "load -- a sync handler making a blocking call ties up an anyio worker thread, and "
          "enough concurrent tool calls exhaust the pool -- which is why it is intermittent, "
          "why their curl and my probe measured opposite things and both were right (my probe "
          "holds a worker, their curl does not), and why CARC 197 tips first at 3.4x the "
          "payload of CARC 29. THE SEQUENCING CONSEQUENCE IS THE POINT: every correctly-routed "
          "appeals call becomes another self-call on a 2-instance scale-to-zero service, so "
          "fixing the misroute in isolation INCREASES the failure rate and will read as a "
          "regression introduced by the fix. Appeals' two changes -- call internal handlers "
          "directly instead of looping over HTTP, and set isError=True on failure -- go BEFORE "
          "or WITH the routing change. Neither is blocked by the chat build hold. ALSO A "
          "FLEET-POWER ITEM I OWN RAISING: a service whose request handlers call themselves is "
          "incompatible with standby/scale-to-zero, the same class of hidden scaling dependency "
          "as mobius-payor's min=1 holding off payer_context degradation. The architecture fix "
          "removes the dependency; until it lands, the scaling posture is load-bearing and "
          "undeclared. "
          "THE STARVATION PREDICTION IS NOW FALSIFIABLE AND PARTLY VERIFIED. Appeals supplied the number: their five MCP tools are declared `def`, not `async def`, so FastMCP runs each in a worker THREAD, while their REST routes are async and the inner self-call is served on the EVENT LOOP -- so the contention is not between outer and inner request, it is purely the outer tool holding a thread while blocking on sync httpx. VERIFIED HERE: anyio's default thread limiter is 40 (current_default_thread_limiter().total_tokens == 40, anyio 4.13.0) and starlette's run_in_threadpool passes NO limiter of its own, so it uses the process-wide default. Against containerConcurrency=80 the service accepts TWICE as many concurrent requests as it has threads to run sync handlers in, and the predicted knee is ~40 concurrent tool calls -- not 80, and not the 2-instance cap. TWO SHARPENINGS BEYOND THEIR VERSION: the limiter is PROCESS-WIDE, not per-tool, so every sync handler in the app competes for the same 40 and a burst of MCP tool calls starves UNRELATED sync endpoints too -- wider blast radius than appeals tools. And the 40 was measured in chat's venv, NOT in the appeals image, so it inherits today's standing caveat: a library default read here is not the default running there. STILL A PREDICTION, NOT A MEASUREMENT. I am DECLINING to run the load test: driving 40+ concurrent requests at a live scale-to-zero service capped at 2 instances would starve it for real users, and it is not my service. It needs the appeals seat's own reproduction against a non-production instance, and it is the evidence every seat agreed to require before anyone writes down a cause for CARC 197. "
          "SUPERSEDED, AND MY OWN CORRECTION WAS HALF RIGHT: IT IS A HOLD-AND-WAIT DEADLOCK AFTER ALL, BY A DIFFERENT ROUTE. I had told Appeals this was starvation rather than deadlock because containerConcurrency is 80, not 1. That killed THEIR reason correctly and my conclusion was still wrong. The sharpening I offered -- that the 40-token limiter is process-wide, so check what ELSE in the app is sync -- is what found the real structure, and Appeals found it: api/app.py:640 `@app.get('/carc-config/{carc}')` is a SYNC route, and it is the second endpoint appeals_lookup_rules calls. So one tool call (a) runs as a sync MCP tool and takes thread token #1 of the process-wide 40, (b) calls /rules/{carc}, async, event loop, no token, (c) calls /carc-config/{carc}, SYNC, needing token #2 WHILE STILL HOLDING #1. Each concurrent call needs two tokens and acquires them sequentially without releasing the first. At 40 concurrent calls all 40 tokens are held at step (a) and every one waits for a token that cannot exist. THE STATE IS ABSORBING, NOT DEGRADED: it never recovers until each blocking httpx call hits its 30s timeout. That fits the evidence BETTER than starvation did -- the observation was `timed out`, not `slow`, and under hold-and-wait latency does not rise smoothly, it goes to exactly the timeout value. It also re-explains CARC 197 WITHOUT payload size: /carc-config is on the path of every appeals_lookup_rules call, so payload may affect how long a token is held but is not needed to explain which CARC tips. REVISED PREDICTION, and cheaper to test than mine: a HARD CLIFF at ~40 concurrent calls with latency pinned at exactly 30s -- far easier to confirm or falsify than a gradual knee. Neither seat reaches this alone: their first instinct was right for the wrong reason, my correction was right about their reason and wrong about the answer, and the question I asked for a different purpose is what exposed it. TWO ADDITIONS. (a) The error message misattributes the call: both fetches sit in one try block, so a /carc-config timeout reports as 'Error fetching rules for CARC 197: timed out', sending any reader to /rules, which answers in 0.15s, to conclude the report is wrong -- another log naming a cause instead of reporting a state. (b) MY SHARPENING-2 CAVEAT IS WORSE THAN I PUT IT: their image has NO requirements.txt. The Dockerfile is `pip install --no-cache-dir fastapi uvicorn pyyaml httpx asyncpg 'mcp[cli]>=1.0,<2'`, so only mcp carries a bound and only a major one; anyio, starlette, fastapi, uvicorn and httpx are whatever pip resolved on 2026-09-08, unknowable from source and liable to move on the next rebuild. So 40 is the value to TEST FOR and cannot be established from the repo -- which makes a pinned requirements file the eighth change, and the one that makes the other seven reproducible. Found 2026-09-10."),
 ("good", "CLOSED 2026-09-10 -- THE JOIN IS EXPLAINED, MY ORIGINAL HYPOTHESIS WAS RIGHT, "
          "AND I WITHDREW IT WRONGLY. Chat Master read the DEPLOYED IMAGE (Artifact Registry "
          "serves manifests and blobs to an access token, so layers pull with curl and open "
          "with tarfile -- no docker, no API enablement, no build). That escape was open all "
          "day and none of us tried it. REPRODUCED HERE INDEPENDENTLY, both halves. (a) The "
          "chat image runs mcp 2.2.0 AND mcp_types 2.2.0 -- I extracted the 213MB layer and "
          "listed the dist-info dirs. requirements.txt:113 says mcp>=1.0.0, which PERMITS 2.x; "
          "we tested 1.0.0, 1.2.0 and 1.26.0 -- THE VERSIONS WE CHOSE, NOT THE ONE DEPLOYED, "
          "and I then told two seats the SDK explanation was 'excluded at every permitted "
          "version'. (b) In 2.2.0 CallToolResult moved to a separate mcp_types package and the "
          "field was RENAMED: `is_error: Annotated[bool | None, Field(alias='isError')]`. "
          "Verified in a scratch venv on mcp==2.2.0: server sends isError=True -> r.is_error "
          "True, hasattr(r,'isError') FALSE, getattr(r,'isError',False) FALSE. That is exactly "
          "what mcp_manager.py:127 evaluates. So: server flags the error correctly -> client "
          "asks for an attribute that no longer exists -> getattr default -> (text, True) -> "
          "success branch -> SourceRef citing the error -> the :2792 text guard passes on the "
          "wrong literal -> the span records success. Every observation accounted for. SCOPE IS "
          "FAR WIDER THAN APPEALS: that getattr is UNCONDITIONALLY FALSE in this revision, "
          "independent of tool, server or error, so EVERY MCP CALL IS RECORDED AS A SUCCESS -- "
          "all 34 tools. tool.dispatched:success does not carry weak information for MCP tools "
          "here, it carries NONE; it is a constant. Appeals is merely where it became visible, "
          "because those are the calls that always fail. AND MY OWN AMENDMENT WAS INERT TOO, "
          "for a second reason: model_fields_set holds FIELD NAMES, NOT ALIASES -- measured, "
          "{'content','is_error'}, so 'isError' in model_fields_set is False and my proposed "
          "check would have shipped and changed nothing, exactly like the getattr-default fix "
          "it replaced. Two inert fixes proposed in one thread, both caught the same way: by "
          "testing against the deployed artefact instead of a local assumption. CORRECTED (1): "
          "pin mcp to 2.2.0 -- a range spanning a MAJOR version is the root enabler, since a "
          "silent major bump renamed a field the code depends on and nothing failed loudly -- "
          "then read result.is_error through the model rather than a getattr default on a name "
          "that is an API contract, failing CLOSED if neither name resolves; then _skill_success "
          "structured; then registry.py:312 sets success=False. THE LESSON IS THE ONE THAT WOULD "
          "HAVE SETTLED IT ON DAY ONE: a version range is not a version, a commit is not an "
          "image, and a local venv is not production -- READ THE THING THAT IS RUNNING. This "
          "also retires Chat Master's dirty-tree candidate: they extracted mcp_manager.py, "
          "mcp_adapter.py, react_loop.py and registry.py from the image and all four are "
          "IDENTICAL to 299519c."),
 ("bad", "OWNER(platform, method): THE DISQUALIFYING FACT WAS FOUR CHARACTERS TO THE LEFT -- A SELF-LABELLED PRINT ASSERTED THE OPPOSITE OF ITS OWN MEASURED VALUE. Chat Master's, and it is a sharper failure mode than the fourth check as we wrote it. Their verification script printed `omitted: r2.is_error = False (None, not False)`. THE MEASURED VALUE WAS False; THE PARENTHETICAL WAS A HARDCODED LABEL THEY HAD TYPED, asserting the opposite. They read the annotation instead of the output, carried it into their doc and into a report, and did so in the message AFTER endorsing the check about resolving disqualifying facts before concluding. So the refutation was not sitting unread in old notes -- it was on the same line as the value. THE GENERALISATION, which is the part to keep: a self-labelled print is a TRAP, because THE LABEL IS WRITTEN BEFORE THE VALUE IS KNOWN and is never re-examined afterwards. The check therefore has to apply to your own FRESH OUTPUT, not only to remembered incidents. RELEVANT TO MY OWN METHOD AND NOT ONLY THEIRS: every probe I ran today printed values next to hand-typed annotations -- `getattr(r,'isError',False) = False  <- mcp_manager.py:127 evaluates this`, `<- MY proposed fix`, `<- the only discriminator`. Mine annotated the MEANING of a value rather than restating the value, which is why none of them inverted; but the distinction is a habit, not a safeguard, and one slip from `<- what this evaluates` to `<- which is None` reproduces their defect exactly. The safe form is to print the value and let it speak, or to compute the label from the value rather than typing it alongside. Found 2026-09-10."),
 ("bad", "OWNER(chat): HOW STEP 2 OF THE FIX MUST ACTUALLY BE WRITTEN -- Chat Master found the durable read and I verified it, then found their supporting claim about omission is wrong in a way that would produce a subtly broken fix. VERIFIED ON THE DEPLOYED 2.2.0: model_dump(by_alias=True)['isError'] returns True when the server sent True. That reads the field by its WIRE name -- the one the MCP spec fixes -- rather than by whatever the SDK currently spells it in Python, so it is correct on 2.2.0 AND on 1.x, i.e. right before the bump, right after it, and right THROUGH it. That is the principle the McpError negative exposed: .error.code survived because it is the JSON-RPC shape and cannot move without a protocol change, while isError moved because it was only a spelling. DEPEND ON THE WIRE CONTRACT, WHICH IS VERSIONED DELIBERATELY AND LOUDLY; NEVER ON A BINDING, WHICH IS VERSIONED SILENTLY. It also gives the derive-from-what-you-own rule the mechanism it lacked: chat cannot own a peer's flag, but it CAN own which NAME it reads that flag by, and the durable choice is the protocol's, not the library's. BUT THE OMISSION HALF DOES NOT WORK AS PROPOSED. Chat Master reported that on 2.x the field defaults to None, so omission is distinguishable from an explicit false, and that a fail-closed-on-omission rule is therefore expressible on 2.x but not 1.x. MEASURED, AND IT IS FALSE: on 2.2.0 `is_error` is `alias='isError' default=False required=False` -- the SAME default as 1.x -- so an omitted flag materialises as False on both, and model_dump(by_alias=True) ALWAYS CONTAINS THE KEY (the default is populated into the dump), which means the dump cannot detect omission AT ALL. Three payloads measured on 2.2.0: sent True, sent false, and omitted give dump values True / False / False with 'isError' present in the dump every time. THE ONLY DISCRIMINATOR FOR OMISSION REMAINS model_fields_set -- and that holds the PYTHON FIELD NAME, not the alias, so it is `'is_error' in model_fields_set` on 2.x and `'isError' in model_fields_set` on 1.x. So the two halves of step 2 pull in opposite directions: THE VALUE must be read by the wire name to survive renames, while STATED-NESS can only be read by the Python name, which is the exact fragility we are removing. Writing step 2 as 'read the protocol field and fail closed when it is not stated' therefore cannot be done through one call, and a fix that used model_dump alone would silently treat every omission as a success -- the current bug, preserved. Workable form: take the VALUE from model_dump(by_alias=True), and for stated-ness either test BOTH spellings in model_fields_set or stop relying on the flag for that question and treat error-shaped content as failure independently. Pinning is what makes the choice knowable either way -- which is Chat Master's conclusion, reached for a different reason than the one they gave. Found 2026-09-10."),
 ("bad", "OWNER(chat): CHAT IS IN THE EXACT STATE THAT ALREADY CAUSED A 29-DAY SILENT "
          "OUTAGE ON THIS FLEET, AND I HAD THAT PRECEDENT WRITTEN DOWN ALL DAY. Appeals answered "
          "the pinning question: YES, they added `<2` on 2026-09-08, commit 06e46d5, whose "
          "message is `pin mcp below 2.x -- the MCP surface had been silently gone for 29 days`. "
          "Before it, b582d7f (2026-07-06) carried the unbounded `mcp[cli]>=1.0` -- which is "
          "exactly the 08-11 source I read from the bucket. My read was not contradicting them; "
          "IT WAS THE PREVIOUS STATE, AND IT WAS THE BROKEN ONE. THE ONSET IS REFRAMED, and the "
          "new version is better than the one I filed twice: I had been reporting `appeals "
          "deployed 09-08, errors start 09-09`. The strong version is that an unbounded mcp "
          "resolved to 2.x, the appeals MCP server stopped working ENTIRELY, and nothing noticed "
          "for 29 days -- health green, no alert, no error rate, because AN ABSENT CAPABILITY "
          "PRODUCES NO FAILURES. The 09-08 commit did not ADD their tools to the merged "
          "manifest, it RESTORED them. So the errors beginning 09-09 are not new behaviour, they "
          "are the FIRST OBSERVATION of behaviour that could not occur while the surface was "
          "down -- whatever was wrong with dispatch was probably wrong the whole time and had "
          "nothing to route. THE PRECEDENT, stated without narrative because it needs none: chat "
          "runs mcp 2.2.0 from an unbounded range TODAY, which is the exact condition that "
          "silently removed a capability from another service on this fleet for a month. That is "
          "documented precedent, not attribution, and it is stronger than the pinning asymmetry "
          "I withdrew. It also means the isError rename is only the VISIBLE half of chat's 2.x "
          "exposure; the 29-day case says the invisible half is worse, because a major bump can "
          "remove a capability outright with every health signal still green. MEASURED AND MY SECOND CLAUSE WAS WRONG -- chat has NO invisible-half exposure, and the reason makes the pinning argument stronger. Chat Master tested chat's mcp import sites against the deployed 2.2.0 and I reproduced it: chat imports exactly two symbols, mcp.client.session.ClientSession and mcp.client.streamable_http.streamable_http_client, at six sites, and BOTH RESOLVE CLEANLY under 2.2.0. Chat never imports mcp.server.fastmcp -- chat is a CLIENT, not a server -- so there is no absent capability waiting. THE CONTRAST IS THE REAL FINDING: a server-side 2.x break is LOUD (ModuleNotFoundError, and 2.x's own message names the rename, links the migration guide and suggests pinning mcp<2 -- I saw it in the traceback), while chat's client-side break is SILENT (renamed field, back-compat alias, getattr default). So the appeals outage COULD ONLY HAVE BEEN MISSED BY SWALLOWING IT -- the SDK practically shouts -- and it still cost 29 days; chat's break cannot be caught that way BECAUSE IT NEVER RAISES. No exception, no warning, no failed import, just a wrong answer on every call indefinitely. Appeals' outage needed a try/except to stay hidden; CHAT'S HIDES ITSELF. The argument is therefore not that chat might lose a capability as appeals did -- it is that the same unbounded range on the same library already produced a month-long invisible failure in a sibling service, and the form it took in chat is the one that cannot announce itself at all. A NEGATIVE FINDING FROM THE SAME SWEEP, worth recording because it distinguishes two things that look alike: mcp 2.2.0 ALSO renamed McpError to MCPError, and mcp_manager.py names McpError at :36 and :47 -- but only in comments. _is_session_error is DUCK-TYPED on structure (getattr(exc,'error').code) rather than on the class, and MCPError in 2.2.0 still exposes .error as ErrorData with .code and .message, so the check STILL FIRES -- verified by constructing MCPError(-32600, ...) on 2.2.0 and running chat's own predicate against it: True. That is the distinction to carry: duck-typing on STRUCTURE that is part of the wire contract survived the rename, while reading an attribute by NAME did not. The docstring reference to `mcp.shared.exceptions.McpError` is now a stale name (doc-only, no behaviour). AND THE PART THAT "
          "IS MINE: I already had this incident in my own notes -- same service, same cause (mcp "
          "2.x renaming `mcp.server.fastmcp`), same 29 days, same green health -- together with "
          "the instruction I wrote for it: `when sweeping for a dependency break, grep for the "
          "IMPORT, not the requirements file -- services with inline pip install in a Dockerfile "
          "and no requirements.txt are invisible to a requirements sweep`. Today's root cause is "
          "mcp 2.x renaming a symbol, in a service with an unbounded range, and I spent the day "
          "reasoning from requirements.txt and locally-chosen versions. I held the answer and the "
          "method the whole time and did not connect them. Found 2026-09-10."),
 ("bad", "OWNER(chat + appeals seat): THE TWO ENDS OF ONE WIRE ARE A MAJOR VERSION APART ON "
          "THE SAME PROTOCOL LIBRARY, AND NEITHER SIDE IS WRONG ABOUT THE PROTOCOL -- THEY ARE "
          "WRONG ABOUT EACH OTHER. Chat Master's final framing, and it is the correct statement "
          "of the seam. chat runs mcp 2.2.0, where CallToolResult exposes `is_error` and "
          "`isError` is a serialization alias only; the appeals prototype runs mcp 1.30.0, where "
          "the attribute IS `isError`. The appeals server sets isError exactly as FastMCP's 1.x "
          "contract requires. Chat reads an attribute its own 2.x SDK renamed. BOTH ARE "
          "INTERNALLY CORRECT AND THE WIRE IS BROKEN BETWEEN THEM. Both depend on `mcp` with NO "
          "UPPER BOUND. THE ASYMMETRY I CLAIMED HERE IS NOW WITHDRAWN, and it is my error twice "
          "over. I said appeals bounded the major (`mcp[cli]>=1.0,<2`) while chat did not "
          "(`mcp>=1.0.0`), so only chat crossed, and that chat should have copied appeals. Chat "
          "Master flagged that I had relayed the appeals half as established when it was the "
          "appeals seat's read of a repo not checked out here, and they could not confirm it "
          "from the image (Cloud Run source deploy leaves NO Dockerfile, requirements.txt or "
          "pyproject.toml in any layer). I then went and got the actual build source from "
          "gs://run-sources-mobius-os-dev-us-central1/services/mobius-appeals-prototype/ and READ "
          "THE DOCKERFILE: it says `pip install --no-cache-dir fastapi uvicorn pyyaml httpx "
          "asyncpg \"mcp[cli]>=1.0\"` -- NO UPPER BOUND. So as of that source, both services were "
          "unbounded and the asymmetry did not exist. TWO CAVEATS THAT KEEP THIS HONEST RATHER "
          "THAN A COUNTER-CLAIM: the newest retained source is 2026-08-11T20:51Z and the "
          "deployed image is tagged pintest-20260908, so THAT SOURCE IS NOT THE BUILD THAT "
          "PRODUCED THE RUNNING IMAGE (no September source exists in the bucket -- checked, zero "
          "objects) and the `<2` may well have been added between the two. Indeed the tag reads "
          "like `pin test`, which would corroborate the appeals seat rather than contradict "
          "them, and appeals landing on mcp 1.30.0 (newest 1.x) rather than 2.x is what a `<2` "
          "bound produces. WHAT IS ESTABLISHED: chat is unbounded (read directly); appeals was "
          "unbounded on 08-11 (read directly); appeals' current bound is UNVERIFIED and "
          "consistent with the installed 1.30.0. WHAT IS NOT: that appeals had a standing "
          "practice chat should have copied. If the bound was added on 09-08 it is days old, not "
          "a habit, and my 'chat should have copied appeals' framing was a narrative built on a "
          "relayed claim. The fix does not depend on any of it -- PIN BOTH SIDES -- but a "
          "one-sided attribution of blame did depend on it. RESOLVED, AND THE WITHDRAWAL SPLITS IN TWO -- one half restored, one half stays withdrawn. Appeals answered with the commit: 06e46d5, 2026-09-08, `pin mcp below 2.x -- the MCP surface had been silently gone for 29 days`. Chat Master independently established it by RESOLUTION TIMING, a better method because it reads the running artefact rather than a file describing an older one; I verified their figures against PyPI: mcp 1.30.0 uploaded 2026-09-07T14:34:14Z, mcp 2.2.0 uploaded 2026-09-07T16:06:19Z (1h32m later), appeals image created 2026-09-08T14:08:27Z with mcp 1.30.0 INSTALLED. 2.2.0 had been on PyPI 22 hours when that image was built; an unbounded >=1.0 takes the newest compatible (exactly how chat got 2.2.0) and a resolver does not pick the second-newest by accident. They also scanned every dist-info METADATA in the image: NOTHING declares a dependency on mcp, so no transitive cap explains it -- the bound could only have come from the install command. RESTORED: appeals' running build bounds the major; chat's does not. STAYS WITHDRAWN: that appeals had a standing practice chat should have copied -- Appeals themselves insisted on this and they are right; the bound is TWO DAYS OLD, was a targeted response to their own outage, and their file is still 5/6 unpinned (fastapi, uvicorn, httpx, asyncpg, anyio unbounded). The fact is established; the blame framing was still wrong. AND THE BUCKET SOURCE I READ IS THE BUILD THAT CAUSED THE OUTAGE, not merely an older one: mcp 2.0.0 shipped 2026-07-28, so the unbounded `mcp[cli]>=1.0` in the 2026-08-11T20:51Z source resolved to 2.x at build time -- and 08-11 to 09-08 is 28 days, which IS the `silently gone for 29 days` window. I read the exact artefact that broke it and mistook it for evidence against the fix. ONE ALTERNATIVE NEITHER OF US HAS EXCLUDED, kept because it is cheap for Appeals to settle: if mcp 1.30.0 were already in the base image or an earlier layer, pip would treat >=1.0 as satisfied and not upgrade, giving 1.30.0 with no bound. A one-day-old release in a base image is unlikely and the git commit makes it near-moot, but nobody pulled the base layers. So the two ends drifted a major version apart and nothing failed "
          "loudly. CONSEQUENCE FOR THE FIX: pin `mcp` on BOTH sides, not just chat's. And it "
          "gives the derive-from-what-you-own rule its final form: 'something chat owns' CANNOT "
          "INCLUDE A FIELD NAME IN A DEPENDENCY THAT A PEER SERVICE RESOLVES INDEPENDENTLY. "
          "Every mechanism chat had for deciding whether a remote tool failed was a value the "
          "peer controlled -- the wire flag, the error wording, the server's own isError -- and "
          "this adds a fourth that is subtler than the three: THE NAME OF THE FIELD ITSELF. "
          "Also the case study for the third standing check (Appeals'): name which side of a "
          "seam produced a behaviour before naming what it costs -- this defect was invisible "
          "from either end alone, because each end was correct on its own terms. Found "
          "2026-09-10."),
 ("good", "SETTLED BY READING THE APPEALS IMAGE -- the anyio-40 figure Appeals said could not "
          "be established from their source IS established, and their deadlock prediction now "
          "rests on a measured constant. Applying Chat Master's image-reading technique to "
          "mobius-appeals-prototype. WHICH ARTEFACT, resolved rather than caveated, since that is this thread's whole lesson: I read it by the tag in the service spec (pintest-20260908), Chat Master read it by digest off the ready revision (@sha256:51b7b29a). The tag RESOLVES TO THAT DIGEST -- docker-content-digest sha256:51b7b29ac610285627f61e5a3c5f030cf106e5d31962ae236d962c29db086ee5 -- and revision mobius-appeals-prototype-00218-zah is latestReady and serving 100% of traffic. Same artefact, and it is the one serving. The image carries anyio 4.15.1, starlette "
          "1.6.0, fastapi 0.141.1, httpx 0.28.1, uvicorn 0.52.4, mcp 1.30.0. I then installed "
          "anyio==4.15.1 in a scratch venv: default thread limiter total_tokens == 40. STRONGER EVIDENCE FOR THE SAME NUMBER, and the distinction matters: Chat Master read CapacityLimiter(40) directly out of anyio/_backends/_asyncio.py INSIDE THE IMAGE, while I installed the same version elsewhere and inferred it. Two methods, one answer; theirs touched the running artefact and is the primary evidence. Nobody should later cite the venv number as the source. So the "
          "40-vs-80 mismatch against containerConcurrency=80 is REAL IN THE DEPLOYED IMAGE, not "
          "assumed from chat's venv, and my own caveat ('a library default read here is not the "
          "default running there') is now discharged rather than outstanding. Their Dockerfile "
          "still pins nothing but mcp, so the figure is true of THIS image and will move on any "
          "rebuild -- their eighth change (a pinned requirements file) remains the one that "
          "makes the other seven reproducible. Coherence check that also passes: appeals runs "
          "mcp 1.30.0, NOT 2.x, which is why their server sets isError correctly on the wire "
          "while chat cannot read it -- the rename bites only the 2.2.0 side. Two services, two "
          "SDK majors, one unpinned range each."),
 ("bad", "OWNER(chat + platform): FOUR SEATS SPENT THE DAY VERIFYING A COMMIT AND NONE OF "
          "US VERIFIED THE IMAGE. With every upstream explanation for the success=True join now "
          "EXCLUDED (the isError field exists at mcp>=1.0.0, the pin floor, and the server sends "
          "it explicitly), Chat Master named the leading candidate and it is our own tooling: "
          "`scripts/deploy.sh dev` builds THE WORKING DIRECTORY, NOT HEAD -- their own "
          "documented gotcha -- in a checkout SHARED BY SEVERAL AGENT SESSIONS. An image built "
          "while any tree was dirty contains that tree. So `git show 299519c:app/services/"
          "mcp_manager.py` proves what the COMMIT contains and says nothing about what the "
          "IMAGE runs, and both of us cited exactly that as evidence. I compounded it: I framed "
          "'the deployed image and this source tree do not behave identically' as a mystery "
          "about upstream drift, when the ordinary explanation is that the image was never "
          "built from that source. This is my own recorded lesson used against me twice in one "
          "day -- point-in-time check is not current state, and the shared-checkout trap -- and "
          "the general form is worse than either: A COMMIT IS NOT A DEPLOYMENT, and `git show` "
          "on a deployed SHA feels like verifying production while verifying nothing about it. "
          "CONSEQUENCE FOR THE INSTRUMENT, which is the actionable part: the log line at "
          "mcp_adapter.py:223 as originally specified (record `success` and `isError`) CANNOT "
          "distinguish 'the code did something unexpected' from 'the code is not the code we "
          "think is running', so it would have burned another cycle. It must also record the "
          "RUNNING MODULE'S IDENTITY -- mcp_manager.__file__, the deployed revision, GIT_SHA -- "
          "plus `'isError' in result.model_fields_set`. Also unexcluded but weak: a producer of "
          "'MCP: <tool>' sources that nobody has found (mcp_adapter.py:274 is the only one "
          "repo-wide, grepped unrestricted). Neither is dressed as a cause; the join stays "
          "OPEN. Five causes have now been named and withdrawn across four seats -- the "
          "manifest example text, routing-alone, tool_agent as caller, 'retry guard is spared', "
          "and my own getattr lead. Found 2026-09-10."),
 ("bad", "OWNER(chat): THE PLANNED isError FAIL-CLOSED FIX HAS NO EFFECT, AND IT WAS THE "
          "LAST STANDING EXPLANATION. Three measurements, mcp 1.26.0 in chat's own .venv. "
          "(1) CallToolResult.isError is a REAL pydantic field with `required=False "
          "default=False`, so a server response that OMITS it is materialised as False BEFORE "
          "any getattr runs. Demonstrated: model_validate of a payload with no isError key "
          "gives r.isError False, and getattr(r,'isError',False) and getattr(r,'isError',True) "
          "BOTH return False. So changing mcp_manager.py:127's getattr default to fail closed "
          "is INERT -- the consolidated fix (1) needs to inspect "
          "`'isError' in result.model_fields_set` (the only way to distinguish omitted from "
          "explicitly-false) or stop trusting the flag altogether. Filed before that code is "
          "written, because it would have looked like a fix and changed nothing. (2) IT ALSO "
          "WEAKENS THE getattr LEAD, WHICH WAS THE ONLY EXPLANATION STILL STANDING FOR THE "
          "success=True JOIN. The primary sends isError EXPLICITLY: isError=True, "
          "'isError' in model_fields_set = True. So the getattr default never fires for this "
          "server -- any SDK version carrying the field parses True, mcp_manager returns "
          "(text, False), and the failure branch attaches NO sources. But we observe sources. "
          "The lead therefore only survives if the DEPLOYED SDK lacks the field entirely, and "
          "Chat Master checked it exists in both mcp 1.26.0 and 1.2.0. I am reporting this "
          "AGAINST my own hypothesis: the join is MORE open than 'one explanation standing', "
          "and the honest state is that no proposed mechanism now accounts for it. (3) THE "
          "APPEALS SERVER'S FAIL-OPEN IS ITS OWN DEFECT, cleanly attributable rather than a "
          "protocol default: it sends isError=false EXPLICITLY "
          "('isError' in model_fields_set = True) on a real backend failure "
          "('Error fetching rules for CARC 197: timed out'). It is not omitting the field and "
          "being defaulted -- it is asserting that a failure succeeded. MECHANISM CORRECTED BY THE APPEALS SEAT, and it makes the defect clearer rather than smaller: their code does not write that flag. Their five tools are @mcp.tool() over FastMCP, whose error contract is LET THE EXCEPTION PROPAGATE -> isError=True, RETURN A VALUE -> isError=False. Every wrapper ends `except Exception as exc: return f[tool] Error: {exc}`, so they catch the failure, convert it to a normal return, and FastMCP faithfully reports the success their code claimed. They are not omitting a flag -- they are SUPPRESSING THE EXCEPTION THAT WOULD HAVE SET IT. THE FIX IS A DELETION, NOT AN ADDITION: remove the blanket except-and-return-string from all five wrappers and let the exception reach FastMCP; that also removes the bare-string-in-the-success-channel hazard, so one deletion closes two defects. It does NOT soften the rule that chat must stop trusting the flag -- Chat Master is right that a server which lies EXPLICITLY defeats model_fields_set too, so we now have a live example of the flag being set and set wrong. Because no client-side check can second-guess an explicit flag, for these five tools THEIR FLAG IS THE ENTIRE CONTRACT with no downstream defence, which moves it from hygiene to the only thing between a backend failure and a cited source. That is the appeals "
          "seat's to fix and does not depend on any chat change. Found 2026-09-10."),
 ("bad", "OWNER(appeals seat + chat): THE ROUTING FIX WOULD NOT HAVE FIXED THE SYMPTOM -- "
          "THE DESTINATION SERVER FAILS OPEN TOO. Found by Chat Master with probe_mcp_tool.py; "
          "reproduced here independently. The appeals prototype advertises all 5 appeals tools, "
          "and call_tool('appeals_lookup_rules', {carc:197}) returns isError FALSE -- attribute "
          "PRESENT, value False -- on a REAL backend failure, with text '[appeals_lookup_rules] "
          "Error fetching rules for CARC 197: timed out'. So call_mcp_tool returns "
          "(error_text, True), the mcp_adapter SUCCESS branch fires, and the timeout is wrapped "
          "as a SourceRef named 'MCP: appeals_lookup_rules' citing itself. Send the appeals "
          "calls to the RIGHT server and the identical pathology reappears with a different "
          "error string. This retires my own recommended fix ORDER, not just a detail of it: I "
          "argued isError-fail-closed first on the grounds that it makes the mystery "
          "reproducible. The real reason is stronger and does not depend on the mystery at all "
          "-- a server we route to in production demonstrably does not set the flag, so "
          "fail-closed is a correctness requirement independent of the join. Note also that "
          "this is a THIRD fail-open, and it is on the OTHER side of the wire: the first two "
          "(getattr isError default at mcp_manager.py:127, and the 'Unknown skill' string guard "
          "at react_loop.py:2792) are chat's; this one is the server's. A client-side "
          "fail-closed check is the only thing that catches it, because the server's own flag "
          "is the thing that is wrong. ALSO A LIVE PRODUCT DEFECT IN ITS OWN RIGHT, unrelated "
          "to the misroute: the appeals CARC 197 rules backend is TIMING OUT RIGHT NOW. CARC 197 "
          "is the exact case that opened this whole investigation, and a timing-out backend is a "
          "better explanation for that turn than either of the two causes I previously asserted "
          "(the manifest's sibling example text, then the misroute). I am NOT declaring it the "
          "root cause -- that would be the third time I have named one on this turn -- it is a "
          "third candidate and the appeals seat owns confirming it. What is measured: the "
          "timeout is real and current, and Appeals' own evidence that /rules/197 returns 5 "
          "rules was taken at a different time, so both can be true. Found 2026-09-10."),
 ("bad", "OWNER(chat): THE LAST LINE OF DEFENCE STRING-MATCHES AN ERROR MESSAGE IT DOES "
          "NOT OWN, AND TWO FAIL-OPENS POINT THE SAME WAY. Found by Appeals; verified here in "
          "one line. react_loop.py:2792 is `_skill_success = env.success and bool(env.text and "
          "not env.text.startswith(\"Unknown skill\"))`, and its own comment at :2791 says it "
          "is 'belt-and-suspenders ... for skills that don't yet set success but still return "
          "an empty or Unknown skill text on failure'. The actual error text is 'Unknown tool: "
          "appeals_lookup_rules'. startswith('Unknown skill') -> False. The guard written "
          "specifically to catch a failure that did not set success does not match the string "
          "it is looking at. THE OFF-BY-ONE-WORD FRAMING UNDERSTATES IT, and the real shape is "
          "worse: registry.py:313 emits `Unknown skill: {name!r}.` -- so the guard matches "
          "CHAT'S OWN literal exactly and always did. What it cannot match is a REMOTE SERVER'S "
          "wording, and 'Unknown tool: <name>' is produced by the roster/credentialing MCP "
          "server, not by chat (chat's own 'Unknown tool' at react_loop.py:3619 sets "
          "success=False and sources=[], so it never reaches this guard). So this is not a typo "
          "-- it is a guard whose contract is a string owned by another system. Renaming skill "
          "-> tool would fix these 21 rows and leave the guard armed only against two exact "
          "spellings, defeated by the next server that words its error differently. Together "
          "with `getattr(result, \"isError\", False)` failing open at mcp_manager.py:127, there "
          "RETRACTION WITHDRAWN -- THE STACKED STORY WAS CORRECT AND I TOLD APPEALS TO DROP IT ON A PREMISE THAT WAS WRONG. I paired this guard with the getattr fail-open, claimed a complete path to success=True with no bug anywhere, then withdrew it after measuring that the primary SENDS isError explicitly -- concluding no SDK could miss it. That inference collapsed two different questions: whether the SERVER SENDS the field, and whether the CLIENT CAN READ it. Reading the deployed image settled it the other way: chat runs mcp 2.2.0, the field is `is_error` with `isError` a serialization alias only, so getattr(r,'isError',False) is UNCONDITIONALLY FALSE in production. BOTH LEGS CONFIRMED, from opposite ends -- I established (a) from the image, Appeals established (b) by reading -- and Appeals had independent evidence for their half and dropped it on my refutation without re-checking it. THE STORY ALSO CLOSES THE ONE THING I HAD LEFT OPEN: I kept saying sources were observed on a branch that should attach none. With success forced True by (a), the adapter's SUCCESS branch fires -- and that is the branch carrying the self-citing SourceRef. The sources are the downstream consequence of the same forced flag; nothing extra is needed and there is no unexplained producer. FORWARD-LOOKING AND THE REASON THIS MUST NOT BE CLOSED WITH THE ROOT CAUSE: PINNING mcp FIXES (a) AND LEAVES (b) ARMED. The :2792 guard will pass silently on any error phrased differently from 'Unknown skill' -- which is every tool except the registry's own unknown-skill path -- so it is sequenced as its own item and must not be signed off as part of the SDK fix. The disproven-then-revived chain was: (1) deployed SDK renames or "
          "drops isError, getattr default reads no-error, success=True; (2) the text guard that "
          "exists to catch exactly that tests the wrong literal and passes; (3) _skill_success "
          "is True and propagates through the result dict, the span "
          "(tool.dispatched -> appeals_lookup_rules:success) and the funnel. TWO DEFAULTS, EACH "
          "INDIVIDUALLY DEFENSIBLE, BOTH POINTING AT SUCCESS -- which is why three seats looked "
          "for one bug and none of us found it. Consequence for the fix order: pinning mcp "
          "addresses (1) and LEAVES (2) ARMED for the next differently-worded error, so the "
          "guard needs to stop matching strings and read a structured outcome instead. Stated as "
          "a reading, not a measurement, on the isError half: the deployed image cannot be "
          "instrumented under the current build hold, and Appeals cannot test it because these "
          "calls never reach their server. The 2792 half IS measured -- the startswith result "
          "above is deterministic. Found 2026-09-10."),
 ("bad", "OWNER(chat): AN HONEST EMPTY ARRIVES DRESSED AS EVIDENCE, AND THIS CREATES A "
          "FIX-ORDERING HAZARD BETWEEN TWO SEATS. The self-citing SourceRef at "
          "mcp_adapter.py:274 is text[:300] of the tool's OWN output under "
          "document_name=f'MCP: {tool_name}' -- the answer citing itself, always exactly 1 on "
          "success. Found by the Governor seat while reviewing my remedy; I had told two seats "
          "to score on len(sources) and that advice was wrong. Three consequences, in "
          "increasing order of how easy they are to miss. (a) Anything scoring groundedness or "
          "quality off len(sources) reads 100% on every MCP turn -- green by construction. "
          "(b) react_retry_guard CANNOT FIRE on MCP tools: _is_zero_result "
          "(react_retry_guard.py:286) requires success=True + signal no_sources + an EMPTY "
          "sources list, and this envelope sets signal='no_sources' AND attaches a source in "
          "the same object nine lines apart. It declares 'no sources' and then supplies the one "
          "thing that stops the no-sources classifier from firing -- the signal field was "
          "already telling the truth and is simply not reachable. So tool-exhaustion never "
          "trips; 16 identical appeals_lookup_rules failures across 16 turns is what that looks "
          "like. I FILED THE OPPOSITE EARLIER ('retry guard is NOT affected, its len(sources) "
          "check saves it') -- that bound was exactly backwards: the check does not save the "
          "guard, it defeats it. (c) THE ORDERING HAZARD, raised by Appeals: their fix removes "
          "the fabricated FL-Medicaid playbook default so a not-found returns an honest empty. "
          "Through the MCP door that honest empty is still TRUTHY TEXT ('{\"found\": false}'), "
          "so `if success and text` holds and it is wrapped as a SourceRef citing itself. Net "
          "effect if their fix lands first: the ANSWER becomes honest and the CITATION still "
          "makes it look sourced -- a truthful non-fact arriving as evidence, which reads as a "
          "VERIFIED ABSENCE rather than a gap. That is harder to spot than the fabrication it "
          "replaces. So the two changes are coupled and the SourceRef must go first or "
          "together; Appeals' fix is necessary and NOT sufficient. Contrast the hardened door, "
          "which fails correctly: react_loop's _no_src() sets success=False, signal=NO_SOURCES, "
          "sources=[] and the guard trips. Same tool, opposite failure semantics -- the fourth "
          "axis of 'the guarantee is conditional on which door you came through', after what it "
          "filters, what it fabricates on a miss, and whether it is reachable at all. "
          "Found 2026-09-10."),
 ("bad", "OWNER(chat): THREE APPEALS TOOLS ARE DISPATCHED TO A SERVER THAT DOES NOT HAVE "
          "THEM, AND THE ERROR IS CITED AS A SOURCE. Discovery is multi-server: "
          "register_mcp_skills (mcp_adapter.py:465-474) merges tools from CHAT_SKILLS_MCP_URL "
          "with tools from every EXTRA_MCP_URLS entry via `discovered = discovered + "
          "extra_tools` -- but _spec_from_mcp_tool builds the SkillSpec from name/description/"
          "inputSchema only, so THE ORIGIN URL IS DROPPED. Dispatch is single-server: the one "
          "call site is `call_mcp_tool(tool_name, _inputs)` (mcp_adapter.py:223) and "
          "_get_mcp_url() (mcp_manager.py:56) takes no tool name -- it returns the constant "
          "MCP_SERVER_URL. So a tool discovered from the appeals prototype is called against "
          "the roster/credentialing server. Verified against the DEPLOYED env "
          "(CHAT_SKILLS_MCP_URL=roster/credentialing/mcp, "
          "EXTRA_MCP_URLS=mobius-appeals-prototype): the primary advertises 29 tools, "
          "appeals_lookup_rules / appeals_get_playbook / appeals_find_carc are ALL ABSENT from "
          "it, get_market_size IS present, and a live call_tool('appeals_lookup_rules', "
          "{carc:197}) returns exactly 'Unknown tool: appeals_lookup_rules' with isError=True. "
          "Live and recent: 21 source rows carrying that error text as a CITED SOURCE -- 16 "
          "appeals_lookup_rules, 4 appeals_get_playbook, 1 appeals_find_carc -- 13 on "
          "2026-09-09 and 8 on 2026-09-10, none before, across 16 turns. That is 21 of 39 MCP "
          "source rows in 30 days (54%). ONE LINK IS NOT CLOSED AND I AM NOT ASSERTING IT: the "
          "only producer of a 'MCP: <tool>' source is mcp_adapter.py:274, which is the SUCCESS "
          "branch, yet mcp_manager.py:127 DOES check isError and returns (text, False) -- and "
          "that check has been in the tree since 0b3505f (2026-02-22), so it is in the "
          "deployed image (299519c8ce). How success became True on an isError=True response is "
          "therefore UNEXPLAINED, and I have now KILLED MY OWN BEST HYPOTHESIS for it. I "
          "assumed the deployed image predated the isError check. It does not, and the proof "
          "is a timestamp: three of the 21 rows land at 14:12:02, 14:14:00 and 14:35:49 on "
          "2026-09-10, AFTER revision mobius-chat-00977-8bl went live at 14:09:05 running "
          "image 299519c8ce -- and `git show 299519c8ce:app/services/mcp_manager.py` contains "
          "the isError check at :127, while `git show "
          "299519c8ce:app/skills/mcp_adapter.py` contains the success-branch SourceRef at :274 "
          "and signal=no_sources at :280. Identical to HEAD. So this is happening in the "
          "CURRENT image, and every failure path in _call_mcp_tool_async returns success=False "
          "(checked all five: isError, ImportError, retriable-exhausted, session-error-"
          "exhausted, generic). Also ruled out: the appeals REST path (react_loop.py:3076) has "
          "a different shape entirely and names no sources, and mcp_adapter.py:274 is the ONLY "
          "producer of a 'MCP: <tool>' document_name in the entire repo. The routing defect and "
          "the cited-error symptom are each confirmed independently; the join needs one log "
          "line on what call_mcp_tool actually returns, which is chat's to add, not mine to "
          "infer. Appeals confirmed the origin side: their server advertises all 5 tools over a "
          "full MCP handshake and has NO log of these 16 turns, so the call never reached them. "
          "Their revision timeline corroborates the onset -- appeals-prototype deployed "
          "2026-09-08 after a gap since 08-11, and the errors start 09-09: the tools became "
          "nameable in the merged manifest and immediately misrouted. Appeals also notes the "
          "5 tools ARE hardened at react_loop.py:3073. RETRACTED -- I then concluded 'react_loop "
          "cannot be the caller', and Chat Master found the mechanism I could not see: THE "
          "TIER-1 APPEALS REST PATH IS DEAD CODE IN PRODUCTION. The universal skill-registry "
          "fallback sits ABOVE it at :2758, guarded only by `_skill_registry.has(tool)` with no "
          "appeals exclusion, and it RETURNS at :2873 -- inside the same function, 200 lines "
          "before the appeals branch. Verified firsthand. Its own comment calls it the fallback "
          "for skills 'not handled above'; appeals is handled BELOW it. So once the name is in "
          "the registry, _execute_tool returns at :2873 and the :3073 branch is unreachable. My "
          "error was specific and worth naming: I verified the branch EXISTS and never checked "
          "whether it is REACHED -- an unconditional set membership proves nothing if control "
          "never arrives. That is feedback_gate_with_no_caller applied to a dispatch branch, "
          "and it means react_loop IS the caller, via the fallback inside the same function. "
          "Re-derived by AST rather than by reading, and it matches Chat Master's numbers "
          "exactly: registry-fallback block 2759-2873, returns at [2859, 2873], LAST STATEMENT "
          "IS AN UNCONDITIONAL Return at 2873; appeals branch 3073-3614, starts after the block "
          "(True), nested inside it (False). has() is true on every instance by either branch "
          "of chat's own log -- 'registered 5 MCP tool(s)' means just registered, 'already "
          "registered' means already held -- and there is NO builtin appeals skill for it to be "
          "held by: `grep -rn appeals_ app/skills/` excluding mcp_adapter.py returns NOTHING. "
          "So the holder is always an MCP registration and dispatch always lands on "
          "mcp_adapter -> call_mcp_tool -> primary. FIX SCOPE IS TWO DOORS, NOT ONE, and this "
          "is the actionable part: tool_agent.py:902 is a genuine second unhardened door (same "
          "bare _skill_registry.has(hint), no appeals interception) but it is NOT needed to "
          "explain the caller, so hardening it alone would leave the MAIN chat path misrouting "
          "exactly as it does today. Both are required: move the appeals branch above :2759 (or "
          "exclude those five names from the registry fallback) in react_loop, AND harden "
          "tool_agent. WHY IT HID: chat's own log says "
          "'skipping MCP tool appeals_lookup_rules -- already registered. Builtins win; MCP "
          "tool not registered' -- and there is NO builtin skill by that name. The appeals "
          "handling is a hardcoded _execute_tool branch that is not in the registry at all. The "
          "actual holder is a PRIOR MCP REGISTRATION from the other server, which :492 "
          "explicitly permits ('Builtin OR prior MCP registration'), so the winner is whichever "
          "server was polled first. A log line naming the wrong reason cost both of us hours. "
          "Chat Master also confirms the blast radius is not cosmetic: the span layer agrees "
          "with the envelope (`tool.dispatched -> appeals_lookup_rules:success`) because "
          "`_skill_success = env.success and ...` at :2792 propagates success=True through the "
          "whole result dict, so planner, retry guard and funnel all saw a successful call. "
          "NEW LEAD on the isError join, still open: production logs 'MCP tool "
          "appeals_lookup_rules completed' -- the logger.info on the SUCCESS return, after the "
          "isError branch -- while the same call from this tree returns success=False. So the "
          "deployed image and this tree do not behave identically at mcp_manager.py:127 despite "
          "the check being in the deployed commit at that exact line and the tree being clean. "
          "requirements.txt:113 is `mcp>=1.0.0`, UNPINNED, and this repo has already been bitten "
          "by SDK drift (698fd0e, streamable_http_client version drift). Independently: "
          "`getattr(result, \"isError\", False)` FAILS OPEN, which is backwards for an error "
          "flag -- a renamed or missing attribute reads as success. SUPERSEDES MY OWN EARLIER ROOT CAUSE for the CARC 197 "
          "offered-and-never-called failure, which I attributed to the manifest's sibling "
          "example text; a tool that cannot be reached is the better explanation and the "
          "example-text finding should be re-tested, not assumed. Found 2026-09-10."),
 ...
 ("bad", "OWNER(chat): THE SOURCE BADGE IS COMPUTED WRONG ON EVERY MCP TURN, AND THE "
          ...
          "signal='no_sources', the identical literal its own failure branch sets at :288. It "
          "is the registry default (registry.py:113) that was never overridden on the success "
          "path, so no MCP tool can ever declare that it found something. integrate.py:733 "
          "then reads that signal FIRST in the badge precedence chain, so one no_sources "
          "poisons the badge for the whole turn -- including turns where corpus search "
          "returned real documents alongside. Measured, 30 days: 206 turns badged no_sources, "
          "of which 157 CARRIED SOURCES; samples include a turn whose first source is "
          "literally 'MCP: get_market_size', and turns carrying 27 and 15 real PDF sources. Of "
          "the 27 turns carrying an MCP source, all 27 compute to no_sources, but only 12 are "
          "BADGED it -- the other 15 escape because integrate.py:1539 lets the model's "
          "source_confidence_override replace the computed badge unconditionally. So the "
          "deterministic computation is wrong 100% of the time here and the user sees a wrong "
          "badge 44% of the time, decided by whether the LLM happened to volunteer an "
          "override. Two scope bounds, both checked: react_retry_guard is NOT affected -- "
          "_is_zero_result (react_retry_guard.py:332-340) also requires an empty sources list, "
          "and these envelopes carry one, so tool-exhaustion does not misfire. And the MCP "
          "adapter does NOT explain the other 145 sourced-but-badged-no_sources turns; those "
          "carry no MCP source and have a separate, still-unidentified cause. Found 2026-09-10 "
          "while checking whether the market/analytics tools were real capability."),
 ("good", "THE MARKET/ANALYTICS TOOLS ARE REAL CAPABILITY, NOT A PROMISE AHEAD OF THE "
          "PRODUCT. This was an open product-truth question: 26 market/analytics tools are "
          "selected, dispatch successfully, and return no_sources every time, and the working "
          "hypothesis was 'the plumbing is real and the data is not'. That hypothesis is WRONG, "
          "and the reason it looked true is the badge defect above. Verified end to end "
          "2026-09-10 against the deployed roster/credentialing service: /market-size returns "
          "2024 FL Medicaid rollups (334 COMMUNITY_BH orgs, $402.4M, 1.386M benes, 5.13M "
          "claims, decomposed intake/high-acuity/ongoing); /rate-benchmarks returns per-HCPCS "
          "p25/p50/p75 payment-per-claim and revenue-per-bene; /market-panel-size returns "
          "monthly panel-size indices by provider type; /market-map returns 188KB of "
          "geo-located orgs with revenue, claims and benes. The chain behind them is real too: "
          "MCP tool get_market_size (provider_skill/mcp/server.py:263) -> fetch_market_size "
          "(utilization_benchmarks.py:3837) -> BigQuery org_profile_v2. Two caveats kept: my "
          "first probes 404'd because these endpoints are POST, not GET -- a method mismatch "
          "is not an absent capability, and I nearly filed it as one; and /org-rate-gap and "
          "/org-radar returned empty for the org slug and name I guessed, which is unresolved "
          "entity resolution on my probe, not demonstrated absence of data."),
...
""", findings=[
 ...
 ("good", "THE THREE STAGE-0 INSTRUMENTS ARE WRITTEN (chat, 2026-09-09), and "
          "building them surfaced three defects that would have made the instrument "
          "LIE. (1) tool.offered now records tool NAMES, sourced from the composer "
          "itself (get_manifest_tool_names) rather than a parallel list that would "
          "drift from what the model actually sees. __unfiltered__ is KEPT alongside "
          "the names, because 'no filter applied' and 'the filter permitted "
          "everything' are different facts about policy and only the sentinel "
          "separates them. (2) _allow(name) returning True DOES NOT MEAN THE TOOL WAS "
          "RENDERED — a registry block can come back empty, so a permitted-but-absent "
          "tool would have read as 'offered'. Recording is now conditional on non-empty "
          "output. (3) _APPEALS_BLOCK IS GATED ON ONE KEY BUT DOCUMENTS FIVE CALLABLE "
          "TOOLS (appeals_find_carc, _lookup_rules, _get_playbook, _validate_claim, "
          "_assemble_letter). Recording only the gate key would have shown "
          "appeals_get_playbook as NEVER OFFERED on the 3 turns the model called it — "
          "the funnel would have reported 'called a tool that was never offered', an "
          "artefact of the instrument rather than a fact about the turn. THE "
          "UNFILTERED MANIFEST IS 28 TOOLS, NOT 24."),
 ("bad", "OWNER(chat): A RECOVERED PARSE FAILURE COSTS A FULL EXTRA LLM ROUND AND "
         "LEAVES NOTHING QUERYABLE. Found by Chat Master 2026-09-09 while splitting "
         "__none__. There IS a format-correction retry, and when it SUCCEEDS the code "
         "only calls emit() — which goes to the turn's thinking chunks, never to logs "
         "or any queryable store. The logger.warning fires ONLY when the retry ALSO "
         "fails. So 'zero parse-failure warnings in 24h' was never evidence of zero "
         "parse failures: it was evidence of zero UNRECOVERED ones, and the recovered "
         "population — the expensive one, one extra round each — was invisible by "
         "construction. This is why Ananth's json-formatting hypothesis could not be "
         "tested, and it is a textbook could-not-check-vs-checked-false: a silent "
         "success path that costs money. Now counted as __unparseable_recovered__, "
         "distinct from __none__ (planner chose not to act) and __unparseable__ "
         "(planner acted, we dropped it)."),
 ("bad", "OWNER(chat): APPEALS AGENT RULED 2026-09-09 — REMOVE THE INVENTED FL "
         "MEDICAID DEFAULT. Ruling, verbatim in substance, four reasons in their order "
         "of severity: (a) A WRONG DEADLINE IS UNRECOVERABLE — most bad answers "
         "degrade an experience, this one LOSES THE CLAIM permanently, and it is being "
         "generated by a service with no payor data at all. (b) IT UNDOES A DELIBERATE "
         "REMEDIATION AT RENDER TIME — the 72 blank FL Medicaid deadlines are blank "
         "BECAUSE the appeals seat removed fabricated ones during P0-0; chat then "
         "refills the gap it finds with an invented default, so the net effect on the "
         "user is IDENTICAL TO NEVER HAVING DONE THE CLEANUP. A data-layer fix made "
         "invisible by the presentation layer. (c) 'DEFAULT FL MEDICAID' IS NOT A "
         "WELL-FORMED CONCEPT — a deadline resolves on a TUPLE (payer, product_line, "
         "state, network_status, audience, appeal_level, request_type) as of the DENIAL "
         "date; FL Medicaid fair hearing, plan appeal, provider claim dispute and a "
         "UM/auth denial are four different clocks, so '60 days' is not a conservative "
         "fallback, it is WRONG ON ITS FACE for most inputs it fires on. (d) IT "
         "VIOLATES A RATIFIED CONTRACT — with Payor Platform: a filing-critical value "
         "below the citable bar is REMOVED, NOT LABELLED, because a 'draft' banner does "
         "not stop a coordinator acting on a number; an UNLABELLED INVENTED one is "
 ...
 ("good", "GATE GREEN ACROSS ALL FIVE COMMITS, 2026-09-09: regressions 0, newly "
          ...
          "OFF: 10133ae carries three behaviour changes plus TWO CHAT FOUND WHILE IN "
          "THERE — `found` was being set from HTTP success when a miss is 200 {}, and "
          "a brief collapse of `success` into `found` WAS CAUGHT BY THE EXISTING SUITE. "
          "test_appeals_playbook_zero_result encodes transport-vs-content and failed "
          "the moment it broke. Consequence if it had shipped: a legitimate 'no "
          "playbook exists' would have recorded as appeals_get_playbook:FAILURE — "
 ...
 ("bad", "OWNER(chat): expires_after_turns HAS NO WRITER, NO COMPARISON, AND ITS "
         "DECREMENT CAN NEVER RUN. Found by Chat Master in the P2 telemetry pass "
         "2026-09-10; I verified it and it is worse than reported. They found it "
         "decremented and never enforced. The full picture: the ONLY assignments in "
         "app/ are the decrements themselves (active_context.py:53, :64); the only "
         "other mention anywhere is a DOCSTRING at context.py:212 describing it as a "
         "field of tool output. So NOTHING EVER SETS IT. Both reads are "
         "ctx.get('expires_after_turns', 0), which returns 0, so the `if ttl > 0` "
         "guard is false and THE DECREMENT NEVER EVEN EXECUTES. A TTL that is never "
         "written, never counted down, and never compared — the context simply never "
         "expires, and the countdown that looks like it manages that is inert in all "
         "three directions. CONSEQUENCE FOR THEIR OWN NEW TELEMETRY, and it is "
         "immediate: expired_but_served=(ttl <= 0) will be TRUE ON EVERY ROW, because "
         "ttl is always 0. That is their own is_fallback finding — a field that can "
         "only hold one value passes 'producer exists', 'consumer exists' and 'query "
         "returns rows' while carrying no information — reproduced in the field they "
         "added hours after warning me about it. The fix is to record the RAW ttl and "
         "whether the key was PRESENT AT ALL, so 'no writer' is distinguishable from "
         "'expired'. Their decision not to make it expire without a ruling is right: "
         "enforcing it changes what the model sees on later turns."),
 ("bad", "OWNER(chat): FOUND IT — carc_group SILENTLY OVERRIDES A CORRECT CARC "
         "CODE, AND GROUP MATCHING IS CASE-SENSITIVE. Reproduced against live appeals "
         "2026-09-10 from a real turn Ananth pasted: 'how do i appeal a carc 197 "
         "denial for sunshine health' returned 'No playbook — using FL Medicaid "
         "defaults'. THE PLAYBOOK EXISTS. "
         "GET /playbook-guarded/Sunshine%20Health/197?audience=provider returns id 55, "
         "carc_group PRECERT, deadline_appeal_days 90, submission portal, fax "
         "1-833-504-0580, mail 'Sunshine Health PO Box 3070 Farmington MO', five "
         "appeal levels. So this is a MISS ON A PAYOR AND CARC THAT BOTH EXIST — which "
         "also retires the CARC-coverage replacement hypothesis for this turn. "
         "THE MECHANISM, at react_loop.py:3201: "
         "lookup = carc_group or str(carc) if carc else carc_group. Python parses that "
         "as (carc_group or str(carc)) if carc else carc_group — so WHENEVER THE MODEL "
         "EMITS A carc_group IT WINS OVER A CORRECT NUMERIC carc. And the endpoint is "
         "CASE-SENSITIVE on the group: verified live, '197' HITS, 'PRECERT' HITS, "
         "'precert' RETURNS {}, 'AUTHORIZATION' and 'auth' return {}. So a model that "
         "emits carc_group='precert' alongside carc=197 — a natural thing for an LLM "
         "to produce — sends 'precert', gets {}, and chat reports NO PLAYBOOK while "
         "holding the correct code. THIS IS THE SPORADIC MISS, and its sporadic "
         "character is exactly explained: whether the turn hits depends on whether the "
         "model happened to emit a group at all. Neither the manifest nor the parser "
         "nor the executor is at fault — the ARGUMENT PRECEDENCE is. FIX: prefer the "
         "numeric carc, or try both, or normalise case — and the manifest never told "
         "the model the group had to be an exact uppercase archetype slug. THE HARM IN "
         "THE OBSERVED TURN: it fell through to rag, produced 90 days plus the PO Box "
 ...
 ("watch", "REPORTED, UNVERIFIED, NOT MINE — DEV AUTH MAY ACCEPT AN EMPTY BEARER. Chat "
           "Master, 2026-09-10, incidentally: /chat/admin/mint-dev-token returns "
           "'access_token' not 'token', so their harness read .get('token','') and sent "
           "'Authorization: Bearer ' with an empty value — AND STILL GOT 200. They "
           "flagged it rather than chased it, and noted the consequence honestly: some "
           "of their earlier 'authenticated' turns were not. I TRIED TO VERIFY AND "
           "COULD NOT — /chat/threads returns 404 with no header, an empty bearer and a "
           "garbage bearer alike, so that path establishes nothing either way. Recording "
           "as REPORTED rather than as a finding, and routing rather than probing "
           "further: an auth boundary is not mine to test by poking a live service. "
           "Belongs to whoever owns chat auth. If it is real it also means any test "
           "result obtained through that path is of unknown provenance."),
 ("bad", "OWNER(chat): THE LEADING L2 HYPOTHESIS — MANIFEST SIZE DEGRADES "
         "SELECTION, AND IT IS CHEAPLY TESTABLE BEFORE ANY RETRIEVAL IS BUILT. "
         "Ananth, 2026-09-10: 'this is the same issue we have had.. we have way too "
         "many tools and react is not able to parse it well.' The post-deploy funnel "
         "makes it concrete: appeals_get_playbook was OFFERED and NOT CALLED on a "
         "question it answers, while the model called appeals_lookup_rules twice and "
         "rag once. And chat found the production manifest offers FIFTY-EIGHT tools, "
         "not the 28 a local render shows — MCP tools register in prod — so the "
         "planner is choosing from roughly twice the catalogue we had measured, on a "
         "prompt where the manifest is already 35.8%-or-more of every token it reads. "
         "THE EXPERIMENT, and it is the whole point of doing it first: re-run the same "
         "query with a REDUCED manifest — the appeals block plus rag, nothing else — "
         "and see whether appeals_get_playbook gets called. Two outcomes, both "
         "decisive. If it IS called, manifest size is the mechanism and P6's retrieval "
         "premise is validated on evidence rather than argument, with a measured "
         "selection delta as its baseline. If it is STILL NOT called, size is NOT the "
         "mechanism and the cause is in the tool's DESCRIPTION or the planner prompt — "
         "which is a different and much cheaper fix, and P6 would have been built "
         "against the wrong theory. This costs one turn and it gates a phase, so it "
         "runs BEFORE the phase. Related: the manifest's own text never tells the model "
         "that appeals_get_playbook is the deadline/submission tool in preference to "
         "appeals_lookup_rules — tool_manifest.py:258-268 says PREFER THIS over rag, "
         "and says nothing about preferring it over its own sibling."
         "HYPOTHESIS LARGELY DISPLACED WITHIN THE HOUR — I READ THE MANIFEST TEXT AND IT INSTRUCTS THE MODEL TO MAKE EXACTLY THIS MISTAKE. tool_manifest.py:250-256, the entry for appeals_lookup_rules, reads: 'USE when: user supplies a CARC number (\'CARC 22\', \'CARC 29\', etc.) and asks HOW TO APPEAL... Use for: \'how do I appeal CARC 22\'... \'rules for CARC 29 timely filing denial FROM SUNSHINE HEALTH\'.' Ananth's query was 'how do i appeal a carc 197 denial for sunshine health' — a NEAR-VERBATIM MATCH for that tool's own documented examples, PAYOR INCLUDED. Meanwhile appeals_get_playbook (:258-268) says only 'PREFER THIS OVER RAG' — over rag, and NOTHING about preferring it over its own sibling. So the model did not fail to parse 58 tools; IT FOLLOWED THE MANIFEST CORRECTLY AND THE MANIFEST IS WRONG. The two appeals tools have overlapping trigger phrasing, one of them claims this exact sentence shape, and nothing disambiguates them. THIS REFRAMES THE NODE: the defect is not selection under load, it is that a hand-written catalogue has no mechanism to keep two sibling entries from claiming the same query — and no test could have caught it, because the manifest is prose. Ananth's size hypothesis is still worth the one-turn reduced-manifest experiment as a control, but it is no longer the leading explanation. AND IT SHARPENS P6: retrieval over a catalogue whose descriptions overlap would retrieve the WRONG tool CONFIDENTLY and faster — the descriptions have to be disambiguated before or as part of retrieval, not after."),
 ("bad", "OWNER(chat): THE MODEL NARRATED A TOOL MISS AS A BROKEN TOOL, IN "
         ...
         "resulted in an Unknown tool error, despite these tools being explicitly "
         "listed as available. This suggests a transient issue or misinterpretation of "
         "the tool availability. RESTRATEGIZE: I will proceed by calling these tools "
         "again.' NO UNKNOWN-TOOL ERROR OCCURRED — both calls reported done and 'Rules "
         "loaded for CARC 197'. The model invented a tooling failure to explain an "
 ...
 ("bad", "OWNER(chat): THE RETRY GUARD DID NOT FIRE ON FOUR DUPLICATE CALLS. Same "
         ...
         "1 AND round 2 with an identical result. react_retry_guard exists at 345 "
         "lines, has NO logs and NO telemetry, and has never once reported firing — "
         "which is indistinguishable from a guard that is not wired. It is one of the "
         "eleven silent nodes in the P2 telemetry work order and this turn is the "
         "argument for taking it early."),
 ("bad", "OWNER(chat): THREE ROUNDS OF 'no chunks marked relevant from the last "
         "result' WHILE THE TOOLS REPORTED SUCCESS. Same turn: every round logged "
         "'Evidence review: no chunks marked relevant from the last result' and the "
         "SAME two gaps stayed open — 'appeal argument for CARC 197', 'submission "
         "method and required documents' — while appeals_lookup_rules reported 'Rules "
         "loaded for CARC 197' twice and rag reported 48 passages, 15 relevant, top "
         "match 73%. So tool output is not reaching the evidence review at all. That "
         "is chat_tool_results (0 rows, only consumer is the DELETE) and curator "
 ...
 ("bad", "OWNER(me): THE READINESS SIGNALS WERE READ FROM A DEAD SESSION "
         ...
         "instruction. gen_chat_dev.py did: sig_path = "
         "'/private/tmp/claude-502/.../scratchpad/readiness.json'; sigs = "
         "json.load(open(sig_path)) IF OS.PATH.EXISTS(sig_path) ELSE {}. That "
         "directory belonged to a session that had ended. The existence check made "
         "the absence SILENT, so every node got {} — and the page rendered "
         "'0 except · 0 log-and-continue · 0 emits' for all 38 as though measured. A "
         "swallowed absence presented as data, in the generator built to find "
         "swallowed absences, on a hardcoded ephemeral path that was GUARANTEED to "
         "rot. FIXED: scripts/platform/gen_readiness.py measures from source on every "
         "refresh (step 2/7, before the merge), and a missing signals file now "
         "sys.exit()s rather than falling back — 'refusing to render zeroed signals as "
         "measurements'. First real numbers: 130 swallow handlers, 3 re-raise, 0 bare "
         "except, 0 un-timed outbound calls, 211 log sites, 144 telemetry sites, and "
         "24 OF 31 LIVE NODES WITH NO STRUCTURED TELEMETRY AT ALL."),
 ("bad", "OWNER(me): MY OWN OBSERVABILITY REGEX MATCHED ONLY logger.info AND NEARLY "
         "SHIPPED 19 MANUFACTURED REDS. The first version of gen_readiness.py used "
         "r'\\b(emit|record|_rec\\w*|span|logger\\.info|track|counter)\\s*\\(' — so a "
         "module full of logger.warning / logger.error read as ZERO emit sites, and "
         "the rubric confidently reported 'emits nothing — cannot be operated' for "
         "NINETEEN nodes including POST /chat (5 logger calls) and state_load (1). "
         "Caught by spot-checking ONE node against grep before shipping, not by "
         "reading the regex. Same class as the reachability transitive-closure error "
         "and the _APPEALS_BLOCK gate-key artefact: a wrong instrument producing "
         "CONFIDENT WRONG VERDICTS, which is worse than no instrument because nobody "
         "re-derives a number that looks measured. FIXED, and the fix is better than "
         "the original intent: log sites and telemetry sites are now counted "
         "SEPARATELY AND NEVER SUMMED, because they are different evidence — a logger "
         "line is greppable by a human after the fact, a span is queryable, "
         "alertable and joinable to a turn. VALIDATION THAT THE INSTRUMENT IS NOW "
         "RIGHT: it independently reproduced a finding established by hand weeks "
         "earlier and recorded at the top of docs/chat-refactor-program.md — 'the "
         "governor contains zero logger calls'. The measurement agrees with the "
         "hand-audit it never saw."),
 ("bad", "OWNER(me): RATINGS WERE NEVER RE-DERIVED, AND NO SIGN-OFF EVER COVERED "
         ...
         "STRUCTURALLY CLOSED 2026-09-09 on Ananth's instruction to write the rubric "
         "and re-rate all 37: ratings are now DERIVED on every refresh by "
         "scripts/platform/rating_rubric.py, so a stale rating is no longer possible "
         "— only a wrong RUBRIC is, and the rubric sits in one arguable place with its "
         "criteria written out. THIRTEEN OF THE THIRTY-SEVEN WERE WRONG, and none of "
         "them looked wrong: 3 red -> amber (defects real but the module IS exercised), "
         "5 amber -> red (defects in modules NO TEST CALLS, where a regression is "
         "silent — jurisdiction, curator_tools, personalization, active_context, "
         "parsing), 1 amber -> red on volume (run_pipeline, 8 open), 1 green -> amber "
         "(stages: zero open bugs but ABSENT coverage — 'no known defects and nothing "
         "tests it' is GREEN BY CONSTRUCTION, the exact shape that hid tool_failed), "
         "and 3 stale badges on DELETED nodes finally reading `removed`. Every rating "
         "now renders its REASON beside it. THE RUBRIC'S STATED LIMITATION, in the "
         "file rather than hidden: it does not weigh severity — a data-integrity "
         "defect and a naming nit both count 1 — because severity is not "
         "machine-readable in this log. So `red` means 'many open defects, or silent "
         "ones', NOT 'the worst module'. Overrides exist and cost something: "
         "rating_override plus rating_override_reason, with the reason rendered, so an "
         "override is a visible judgement rather than a quiet correction."),
 ...
 ("bad", "OWNER(me): THE COVERAGE ENUM WAS A PRODUCER WITH NO CONSUMER, IN THE TOOL "
         ...
         "accurately' to find it. docs/chat-coverage.json had existed for a day "
         "carrying Eval's Layer-1 state per node, and THE PAGE NEVER RENDERED IT: 'is "
         "this node guarded?' was answerable only by opening a JSON file by hand. "
         "Worse, it could not have worked even if the page had tried — gen_coverage.py "
         "ran at step 5/6, AFTER the page was written at 3/6, so any read would have "
         "been of the previous run's file. AN ORDERING DEFECT IN THE CHAIN THAT EXISTS "
         "TO PREVENT ORDERING DEFECTS, and the second one in it: "
         "gen_chat_submodules.py was missing from the same chain for hours and the "
         "page drew a deleted use_react branch. FIXED 2026-09-09: coverage moved to "
         "3/6 with the reason in a comment, merged into each node by gen_chat_dev.py, "
         "and rendered as a `test:` pill carrying the enum plus the contract tags. "
         "VERIFIED IN A BROWSER, not by grep — state_load renders 'test: "
         "TAGGED-UNVERIFIED' with state_load:no_silent_reset, jurisdiction renders "
         "'test: ABSENT'. And the render check earned itself immediately: my first "
         "version put an apostrophe (Eval\\'s) inside a single-quoted JS string and "
         "node --check caught the SyntaxError before it shipped — the same defect "
         "class as the blank page I shipped earlier in this program, caught this time "
         "because running the artifact is now the verification step rather than "
         "grepping it."),
 ("bad", "OWNER(chat): THE STRONGEST CANDIDATE YET FOR THE SPORADIC MISS — THE "
         "PAYOR KEY IS FREE TEXT FROM THE MODEL, MATCHED EXACTLY, AND A WRONG GUESS "
         "IS BYTE-IDENTICAL TO 'NO PLAYBOOK EXISTS'. Found 2026-09-09 by following "
         "appeals' lookup gotcha. Appeals demonstrated: "
         "/playbook-guarded/Sunshine%20Health/29 returns a 2,707-byte object; "
         "/playbook-guarded/sunshine_health/29 returns 200 and {}. The key is the "
         "DISPLAY NAME, matched exactly. Chat does NOT snake_case it — verified, that "
         "hypothesis is disproved — but what it does is worse: payor comes straight "
         "from inputs.get('payor') at react_loop.py:3154, i.e. WHATEVER THE MODEL "
         "WROTE, inferred from the user's phrasing. And tool_manifest.py:258-268 tells "
         "the model nothing about the required form — no enumeration, no 'must be the "
         "exact display name', no list of valid payors. So 'Sunshine', 'sunshine "
         "health', 'Sunshine Health Plan' and 'Molina Healthcare of Florida' all "
         "return {} indistinguishably from a genuine miss, and chat then reports NO "
         "PLAYBOOK for a payor that has one. THIS PRODUCES THE EXACT REPORTED SYMPTOM "
         "AND ITS SPORADIC CHARACTER: whether the call hits depends on how the USER "
         "happened to phrase the payor's name. Ananth's original framing — 'tool "
         "selection is sporadic and the model narrates a miss as a broken tool' — is "
         "satisfied by this mechanism without any manifest, JSON-parsing or executor "
         "fault. NOT YET CONFIRMED as the cause of any specific turn; it is a "
         "mechanism with a demonstrated failure mode, and it is now the first thing "
         "the instruments should be pointed at. RETIRED ON EVIDENCE 2026-09-09 — THE "
         "DATA POINTS THE OTHER WAY, THREE WAYS. Chat ran the experiment "
         "RETROSPECTIVELY rather than waiting for the held instrument: "
         "chat_turns.thinking_log persists the emit strings, and "
         "'Checking {payor} playbook...' carries the payor verbatim while the next "
         "emit carries the outcome. Window 2026-02-06 to 2026-09-10, 5,864 turns with "
         "a thinking_log, 153 mentioning a playbook, 121 PAIRED payor->outcome "
         "observations — far wider than the span data would have given. Results: "
         "Sunshine Health usable 88 / EMPTY 23; FL Medicaid usable 6 / EMPTY 1; "
         "sunshine-health USABLE 2; placeholder_payor EMPTY 1. THE HYPOTHESIS "
         "PREDICTED EMPTIES CLUSTER ON NON-EXACT NAMES. (1) 'sunshine-health' — "
         "hyphenated, lowercased, NOT the display name — returned USABLE BOTH TIMES, "
         "which is close to fatal on its own: exact matching is not required, or "
         "something normalises upstream. (2) 'FL Medicaid', not a real payor per "
         "appeals' corpus, returned usable 6 of 7. (3) 23 OF THE 25 EMPTIES ARE ON "
         "'Sunshine Health', THE EXACT DISPLAY NAME — empties cluster on the CORRECT "
         "string. So this is a retirement, not a failure to confirm. MY REASONING WAS "
         "SOUND AND THE MECHANISM IS REAL (appeals demonstrated sunshine_health -> "
         "200 {}); it is simply not what is happening on live traffic, which is what "
         "measurement is for. LEADING REPLACEMENT, chat's, stated as a hypothesis not "
         "a finding: a playbook is keyed (payor x CARC) and appeals covers 72 distinct "
         "CARCs, so a CARC outside that set is a LEGITIMATE miss — exactly 143309c1 "
         "(CARC 24, absent), which appeals independently closed as correct. NOT "
         "PROVABLE from this data because the emit carries the payor but not the CARC; "
         "the held instrument records `lookup` alongside the payor and settles it in "
         "one query. That is now the only remaining reason to deploy it. FIX "
         "CANDIDATES (no longer indicated): enumerate valid payors "
         "in the manifest, resolve the name server-side against the registry, or have "
         "appeals return a distinguishable 'payor unknown'. Also note "
         "react_loop.py:3170 f-string-interpolates payor into the path with NO URL "
         "ENCODING.\n\nDISPATCHED 2026-09-09 on Ananth's instruction, two halves: "
         "(1) CHAT LOGS THE PAYOR STRING ACTUALLY SENT — verbatim, unmodified, paired "
         "with whether the response was {} — and CONFIRMS or RETIRES this candidate on "
         "data. Grouped by distinct payor string with counts; a bare count of empties "
         "proves nothing since we already know empties happen. Cross-checkable against "
         "appeals' 141 rows, which turns 'some calls come back empty' into 'N calls "
         "came back empty for payors that HAVE playbooks'. (2) APPEALS RESOLVES THE "
         "NAME VIA THE LEXICON's j:payor.<slug> axis rather than requiring an exact "
 ...
 ("bad", "OWNER(me, rag): DISPLAY NAME IS DERIVED FROM THE SLUG BY TITLE-CASING, AND "
         ...
         "'Sunshine Health' happens to work; it mangles the moment a real payor name is "
         "irregular — 'Molina Healthcare of Florida' (lowercase 'of'), an ampersand, a "
         "hyphen, 'UnitedHealthcare'. Slug and display name are TWO IDENTIFIER SPACES "
         "with no reliable mechanical conversion between them, and I am deriving one "
         "from the other. Surfaced 2026-09-09 while offering the Lexicon to the appeals "
         "seat as their payor resolver — flagged to them explicitly so they would not "
         "copy the pattern and inherit it. The canonical slug<->display-name mapping "
         "belongs in the Payer Reference-Data Store and should be LOOKED UP, NEVER "
         "DERIVED; RAG already calls /api/registry/payors/<display_name>/web-domain, so "
         "slug -> canonical display name is the missing half of an endpoint pair that "
         "already half-exists. Mine to fix.\n\nESCALATED 2026-09-09, AND IT IS NOT "
         "ONLY MY MODULE — THE SAME DERIVATION IS INSIDE THE RESOLVER BEING PROPOSED "
         "AS THE FIX. GET /api/registry/payer-canonical on mobius-payor exists and is "
         "live (registry_admin.py:228) — the endpoint I proposed building and the "
         "appeals seat was about to request. But _canonicalize_one's TIER-2 LEXICON "
         "PATH does, at registry_admin.py:203: canonical = "
         "key.replace('_', ' ').title(). That is byte-for-byte the defect I flagged in "
         "payer_context.py:134. CONSEQUENCES, and they invert the plan: (a) every "
         "via='lexicon' canonical name is a SYNTHESISED STRING, never a looked-up one — "
         "so 'Molina' -> 'Molina Healthcare' is derived, not authoritative; (b) a "
         "lexicon key 'fl_medicaid' would resolve to 'Fl Medicaid', which is NOT "
         "appeals' stored display name 'FL Medicaid', so wiring the resolver in WOULD "
         "STILL MISS on 51% of their corpus — and they would reasonably conclude the "
         "Lexicon lacked coverage when the derivation had mangled it. (c) The registry "
         "TIER-1 half is sourced from payor_readiness_asset — a table about READINESS "
         "ASSETS — so any payor without a readiness asset can only ever resolve "
         "through the mangling tier. Also worth stating: the docstring scopes this "
         "endpoint to 'ingest/publish'; using it at runtime on a FILING-CRITICAL path "
         "is a scope extension that the payor seat should ratify rather than inherit."
         "\n\nWORSE THAN THE MANGLING — THE MATCHER HAS NO RESIDUAL CHECK, SO IT "
         "RETURNS A STRICT GENERALISATION OF THE INPUT AND REPORTS resolved:true. "
         "Found by the appeals seat 2026-09-09, verified firsthand here at "
         "registry_admin.py:207-214. ?value='Molina Healthcare of Florida' -> "
         "{canonical: 'Molina Healthcare', j_tag: 'j:payor.molina_healthcare', "
         "resolved: TRUE, via: 'lexicon'}. 'of Florida' is silently discarded. THE "
         "MECHANISM, precisely: pass 1 iterates sorted(lex_keys, key=len, reverse=True) "
         "under a comment saying 'most specific wins first' — which is true of KEY "
         "ordering and gives false comfort, because it says nothing about the INPUT. "
         "The match is re.search(r'\\b' + spaced + r'\\b', norm), a SUBSTRING match "
         "against a longer input, and NOTHING CHECKS THAT THE MATCH CONSUMED THE "
         "INPUT. There is no residual test. So a key that is a prefix of the input "
         "wins and the remainder is dropped without a trace. WHY THIS IS WORSE THAN A "
         "MANGLED NAME: mangling FAILS LOUDLY — you get a miss and go looking. This "
         "SUCCEEDS CONFIDENTLY AT THE WRONG GRANULARITY. Molina Healthcare of Florida "
         "is a Medicaid MCO under 42 CFR 438 with its own appeal chain, deadlines and "
         "submission channels; Molina Healthcare is the national brand. The resolver "
         "merges them and labels it resolved. It is the SAME product_line/state "
         "collapse the fact-store composite key exists to prevent, arriving UPSTREAM "
         "of the key and PRE-LABELLED as success. CONSEQUENCE for any consumer: a "
         "via='lexicon' result is not merely lower-authority, it may be a STRICT "
         "GENERALISATION — never key another lookup on it, and any "
         "surface-for-confirmation must show the ORIGINAL STRING beside the "
         "resolution or the user confirms a narrowing they cannot see. FIX: refuse "
         "resolved:true when the match leaves a meaningful residual; return the "
         "residual so the caller can see what was dropped."
         "\n\nCORRECTED 2026-09-09, AND THE CORRECTION REVERSES THE ASK. Appeals "
         "tested tier 1 and I verified it: ?value='aetna better health of florida' -> "
         "canonical 'Aetna', resolved TRUE, via='REGISTRY' — not lexicon. So BOTH "
         "TIERS GENERALISE. _resolve_payor (mobius-payor/app/skills.py:167) does the "
         "same residual-free match and ITS DOCSTRING STATES THE GENERALISATION AS "
         "INTENT: 'Every variant of a brand (\'Aetna Better Health of Florida\', "
         "\'Aetna Florida\', \'Aetna\') contains the j-tag key (\'aetna\'), so we "
         "match the payor whose j-tag key appears in the query.' It scores by "
         "len(cand) — longest CANDIDATE, never residual — and its inner test is "
         "`re.search(\\b cand \\b, ql) or cand in ql`, the second arm UNANCHORED, so "
         "it is looser still. THEREFORE THIS IS NOT A BUG, IT IS A SEMANTIC MISMATCH. "
         "The endpoint answers 'which brand family is this document about' — correct "
         "and necessary for ingest/publish, which is its declared consumer and which "
         "needs every Aetna variant collapsed onto one canonical payor to stamp a "
         "corpus. A filing path needs a different question: 'which contracting entity "
         "adjudicated this claim'. Only one of those can be right per endpoint. A "
         "residual check imposed as a FIX would BREAK RAG's publish path to serve a "
         "consumer that is not using it yet. SO THE ASK IS A MODE, NOT A FIX: a "
         "STRICT resolution that refuses resolved:true on a meaningful residual and "
         "RETURNS the residual; default behaviour unchanged; flag, parameter or "
         "sibling endpoint at the payor seat's discretion. ALSO CORRECTED: `via` "
         "distinguishes DERIVED from LOOKED-UP, which is real and useful, but it does "
         "NOT distinguish EXACT from GENERALISED — and on a filing-critical path the "
         "second is the one that matters. Any consumer treating via='registry' as "
         "safe-to-proceed is wrong, including my own earlier instruction to do so."),
 ("bad", "OWNER(payor seat): THE _GENERIC BLOCKLIST MAKES PASS 2 UNREACHABLE FOR ANY "
         "PAYOR KEY WHOSE FIRST TOKEN IS A BRAND WORD — AND FLORIDA IS ON THE LIST. "
         "registry_admin.py:218-224: brand = key.split('_')[0]; if brand in {'florida', "
         "'community', 'health', 'healthcare', 'care', 'national', 'blue'}: continue. "
         "So florida_blue and florida_medicaid can never resolve through pass 2. "
         "FLORIDA BLUE IS A MAJOR REAL PAYOR WHOSE BRAND TOKEN IS LITERALLY ON THE "
         "BLOCKLIST, and the appeals pilot is entirely Florida — the cure has a "
         "systematic blind spot exactly where the product operates. The intent is "
         "sound and the comment defends a real false match ('florida community care' "
         "must not claim 'Molina Healthcare of Florida'); the implementation makes a "
         "geography-shaped hole. CORRECTION TO THE REPORTED EVIDENCE, mine: appeals "
         "cited ?value='florida medicaid' -> resolved:false as the demonstration, but "
         "that does NOT establish the blocklist as the cause — pass 1 matches on the "
         "SPACED key, so if florida_medicaid were a lexicon key, 'florida medicaid' "
         "would have resolved via pass 1 regardless of pass 2. The observed miss is "
         "more likely KEY ABSENCE. The blocklist defect is real and reachable by "
         "reading the code; that particular test does not prove it. Distinguishing "
         "them matters because the fixes are different — one is a matcher change, the "
         "other is Lexicon coverage, and mine. RESOLVED 2026-09-09: appeals ran the "
         "discriminating test and IT IS COVERAGE, MINE. 'florida community care' "
         "resolves TRUE via lexicon — a florida-first key clearing pass 1 with the "
         "blocklist never consulted — while 'florida blue' and 'florida medicaid' both "
         "return false. So florida_community_care EXISTS as a key and the other two "
         "DO NOT: key absence, not matcher blindness. LEXICON COVERAGE DEBT, MINE: "
         "florida_blue and florida_medicaid are missing, and Florida Blue is a major "
         "real payor in the appeals pilot's only geography. The blocklist remains a "
         "genuine latent defect for brand-token-only inputs ('blue' unresolved while "
         "'molina' and 'sunshine' both resolve) but is LOWER PRIORITY than reported, "
         "and its stated rationale is already moot: the comment defends against "
         "'florida community care' claiming 'Molina Healthcare of Florida', which "
         "pass 1 handles correctly on its own."),
 ("bad", "OWNER(chat): `usable` PASSES THE ONE CASE IT WAS BUILT TO CATCH. Appeals "
         "measured it against live data: FL Medicaid x 29 returns "
         "{deadline_appeal_days: null, submission_method: 'portal', portal_url: '', "
         "fax: '', mail_address: ''}. Chat's guard is "
         "usable = found and (days is not None or bool(method)) -> False or True -> "
         "TRUE. So the '?d deadline' shape — 72 OF 141 ROWS, 51% OF THE CORPUS, all FL "
         "Medicaid — is admitted as usable, and chat proceeds to answer a DEADLINE "
         "question from a playbook that has no deadline. The predicate is a single "
 ...
 ("bad", "OWNER(appeals-agent, self-reported): '51% OF THE CORPUS' WAS ONE "
         ...
         "signature across all 72; every payor-specific scalar blank "
         "(deadline_appeal_days None, portal_url '', fax '', mail_address ''). So it "
         "is a TEMPLATE, not 72 sourced records, and it carries no payor identity at "
         "all. THE DECISIVE EVIDENCE THAT IT IS ALSO NOT THE AGENCY: ladder level 5 is "
         "'AHCA Complaint / CMS — File with FL AHCA or CMS. Last resort.' YOU CANNOT "
         "ESCALATE TO THE AGENCY IF YOU ARE THE AGENCY — and level 4, 'Provider Claim "
         "Dispute — contractual right', presupposes a contract with a PLAN. So 'FL "
         "Medicaid' is neither AHCA nor fee-for-service Medicaid: it is a generic "
         "managed-care-plan appeal template stored under a payor-shaped name. "
         "CONSEQUENCES, three of them retractions of my own filings: (1) the FL "
         "Medicaid IDENTITY RULING I asked the payor seat for is WITHDRAWN — it is not "
         "a payor, so there is nothing to canonicalise, and point 3 rules out the "
         "AHCA mapping on the template's own evidence; (2) the Lexicon coverage debt "
         "shrinks — florida_medicaid should NOT become a key, and appeals' real payor "
         "list is SUNSHINE HEALTH, SINGULAR; (3) every '72 of 141 rows / 51% of the "
         "corpus' statement in this log, mine included, described one template as "
         "though it were 72 records with a sourcing gap. The honest statement is ONE "
         "REAL PAYOR AND ONE DEFAULT. Appeals' fix, theirs: re-key those rows to the "
         "actual MCOs or mark them as the template they are."),
 ("good", "PAYOR SEAT GRANTED BOTH RULINGS AND SHIPPED THEM, 2026-09-09 "
          "(mobius-payor b358aa7) — VERIFIED FIRSTHAND IN THE CODE, at their own "
          "request not to treat their message as the deploy. (1) A PROGRAM IS NOT A "
          "PAYOR: 'FL Medicaid' now returns entity_type='program', state FL, program "
          "Medicaid, administered_by AHCA, member_payors [Aetna, Sunshine Health], "
          "resolved:false, via='program_namespace' — namespaces DERIVED from "
          "health_plan rows, never hardcoded. Their grain answer: payors are ROWS, "
          "programs are NAMESPACES OVER THEM. And their own data agreed all along — "
          "AHCA's payor_type is state_agency and its lexicon_code is "
          "regulatory_authority.ahca, NOT payor.*; the registry always knew AHCA was "
          "not an ordinary payor and the resolver never read it. Guidance for appeals' "
          "72 template rows: fair-hearing/floor playbooks key to the PROGRAM (the "
          "regulatory floor every member payor inherits), plan-appeal chains key to the "
          "MEMBER PAYOR. (2) IDENTITY NOW COMES FROM THE IDENTITY TABLE — tier 1 reads "
          "the payor spine (verified: 'FROM payor' at registry_admin.py:253), not "
          "payor_readiness_asset, because readiness is whether we have assets FOR a "
          "payor and never WHO the payor is; a payor with zero readiness rows now "
          "resolves. .title() canonicals are GONE STRUCTURALLY: the only surviving "
          "occurrence (:279) feeds display_hint, explicitly labelled "
          "authoritative=False. A lexicon-only hit returns resolved:true, "
          "canonical:NULL, slug + display_hint. (3) RUNTIME USE RATIFIED and the "
          "docstring now says RUNTIME-GRADE — while holding that appeals' non-blocking "
          "+ resolver_unavailable design should stay REGARDLESS, on their own standing "
          "note that mobius-payor availability was once held up only by "
          "min-instances. BREAKING CHANGE FOR MY payer_context SWITCH: lexicon-only "
          "hits NO LONGER CARRY A CANONICAL STRING AT ALL — branch on `authoritative`, "
          "use display_hint for DISPLAY ONLY, never for keying."),
 ("good", "STRICT MODE SHIPPED AND VERIFIED, 2026-09-09 (mobius-payor 2c484f0). "
          "?mode=strict on GET, body.mode on batch; lenient default untouched. Read "
          "firsthand: _residual() at registry_admin.py:207 with the docstring 'the "
          "evidence the match GENERALISED. Empty residual = exact-scope match'; strict "
          "returns resolved=False, refused='generalised_match', the residual, AND "
          "`candidate` carrying what lenient would have said — so a filing caller can "
          "escalate to a human with the whole picture rather than just a refusal. "
          "'sunshine health of florida' strict-refuses on residual ['florida']; exact "
          "names resolve identically in both modes. This closes the only real ask left "
          "from an evening that generated four of my own retractions."),
 ("bad", "OWNER(me + payor seat): THE ENDPOINT AT THE CENTRE OF THIS WHOLE "
         "INVESTIGATION HAS NO CONSUMER, AND ITS DOCSTRING NAMES ONE. Verified "
         "2026-09-09 after the payor seat mentioned it in passing: the batch "
         "endpoint's docstring (registry_admin.py:360) reads 'For RAG's publish.py / "
         "corpus-cleanup republish', and mobius-rag has ZERO callers of "
         "payer-canonical — the only grep hit is an unrelated migration's print "
         "string. TWO CONSEQUENCES, and the first is against me. (1) MY CENTRAL "
         "CAUTION WAS PROTECTING A PATH THAT DOES NOT EXIST. I told the payor seat not "
         "to add a residual check because it 'would break RAG's publish path to serve "
         "a consumer that isn't using it yet' — correct as a principle, and the "
         "specific risk was zero, because publish.py never wired up. I inferred a live "
         "consumer from a docstring, which is the exact error I have filed against "
         "others twice today. (2) THE GOVERNANCE PICTURE CHANGES: the endpoint has no "
         "live consumer at all, so appeals would be its FIRST, the lenient default is "
         "protecting a hypothetical rather than a running path, and the payor seat "
         "could have changed the default freely. Worth them knowing. AND IT IS THE "
         "THIRD INSTANCE OF THE SAME FAMILY IN ONE EVENING — after the untracked "
         "handoff and _party_suppressed — except this one is the sharpest: an artefact "
         "that DOCUMENTS its consumer, which is what made two seats believe the "
         "consumer existed. A named consumer reads as evidence of connection and is "
         "not.\n\nCORRECTED 2026-09-09 — MY 'NO LIVE CONSUMER AT ALL' WAS WRONG, AND "
         "WRONG IN THE EXACT WAY I HAD BEEN FILING AGAINST OTHERS ALL EVENING. A live "
         "consumer exists: provider-roster-credentialing's /org/resolve at "
         "mobius-skills/provider-roster-credentialing/provider_skill/"
         "org_handler.py:1006-1015 — GET, lenient default, j_tag-primary, canonical "
         "used for display_name, timeouts and non-200 treated as no-match. Verified "
         "firsthand. MY ERROR: I grepped mobius-rag, found zero, and reported 'no live "
         "consumer at all, blast radius empty'. A grep scoped to one repo cannot "
         "answer a question about all repos — this is filter-makes-the-answer-look-"
         "complete, committed by me, hours after I filed the same shape against two "
         "other seats. CONSEQUENCE: the blast radius was NOT empty, and my ORIGINAL "
         "caution about not breaking the generalising match was ACCIDENTALLY CORRECT — "
         "/org/resolve depends on lenient generalisation for brand variants. I was "
         "right for a reason I had not found, then wrong when I went looking. THE "
         "RECURSIVE INSTANCE, theirs and self-reported: they committed a docstring "
         "reading 'NO LIVE CONSUMER EXISTS — verified by grep' WHILE THE DISPROOF SAT "
         "IN THEIR OWN GREP OUTPUT, the hit being from mobius-skills and outside the "
         "pattern they were mentally filtering for. Corrected within the hour "
         "(mobius-payor 239bdfe), and the docstring now lists consumers with file:line "
         "and keeps BOTH wrong claims on the record. So the fix for 'a named consumer "
         "with no wiring' was itself 'a verified-absent claim with the presence in "
         "view'. HABIT ADOPTED, with teeth: A CONSUMER CLAIM CARRIES ITS file:line OR "
         "CARRIES NOTHING — and a grep's SCOPE is part of its result."),
 ("bad", "OWNER(roster seat): PAYOR'S canonical:null CHANGE BREAKS display_name IN "
         "provider-roster-credentialing, AND .get's DEFAULT DOES NOT FIRE. "
         "org_handler.py:1015 does data.get('canonical', name) — but Python's .get "
         "default only fires when the key is ABSENT, and payor now returns the key "
         "PRESENT-AND-NULL for lexicon-only payors. So display_name becomes None where "
         "it used to be a title-cased string. SCOPED PRECISELY, because it changes "
         "urgency: line 1012 does the same .get inside _slug(), but it is guarded by "
         "the j_tag branch and payor STILL SETS j_tag on the lexicon-only path "
         "(registry_admin.py:313), so _slug(None) is never reached and there is NO "
         "CRASH — the impact is a None display value, not an exception. Fix, one line, "
         "theirs: data.get('canonical') or data.get('display_hint') or name. The payor "
         "seat is notifying them directly. WORTH NAMING AS ITS OWN CLASS: dict.get "
         "with a default is a NULL-SAFETY ILLUSION — it guards absence, never "
         "presence-of-null, and the two are indistinguishable at the call site. Any "
         "consumer written against an API that may start returning explicit nulls has "
         "this bug latent, and payor's change from a synthesised name to an honest null "
         "is exactly the kind of correctness improvement that triggers it."),
 ("bad", "OWNER(payor seat / whoever owns intake): TWO SCRIPTS ARE NAMED LIKE TESTS "
         "AND ONE OF THEM ABORTS THE WHOLE PYTEST COLLECTION. Flagged by the payor "
         "seat as an observed fact; I reproduced it and the cause is NOT the file they "
         "guessed. tests/test_intake_usecases.py:231 has a MODULE-LEVEL "
         "`sys.exit(asyncio.run(main()))`. pytest EXECUTES module-level code during "
         "collection, so that line raises SystemExit and collection dies with "
         "INTERNALERROR after 52 tests — the suite cannot be run at all on a clean "
         "tree. tests/test_parse_failproof.py has the same shape. They attributed it to "
         "the research-console/ask-modal commits (9dd21c3, 1da5c1e); the actual "
         "offending file came in at c8f371a. Worth correcting because the wrong "
         "attribution sends the fix to the wrong author. FIX: these are standalone "
         "scripts, not tests — move them out of tests/ or guard the entrypoint behind "
         "`if __name__ == '__main__':`. THE CLASS: a script whose FILENAME claims it is "
         "a test, which is the appeals seat's identity-not-path pattern applied to "
         "code — it reads as a test to every tool that globs tests/, and its real "
         "effect is to disable the gate it appears to be part of. The payor seat "
         "verified their own change via live TestClient probes instead, which is why "
         "this was survivable rather than blocking. CLOSED 2026-09-09, mobius-payor "
         "7c23797 — VERIFIED FIRSTHAND: collection is clean and reports 146 TESTS, up "
         "from 52-plus-an-abort. Both entrypoints guarded with if __name__ == "
         "'__main__' (mechanical; the test logic untouched). Clean run: 118 passed / 4 "
         "failed / 24 skipped, and the 4 failures are all inside the new files "
         "(test_research_identity x3, test_runner_contract x1) — PRE-EXISTING, their "
         "authors' to triage, and NOW VISIBLE instead of hidden behind an aborted "
         "collection. Both offending files came from c8f371a, today, another session "
         "under Ananth's git identity — so my attribution correction mattered: the "
         "payor seat had named the research-console commits and would have routed the "
         "fix to the wrong author. The payor seat also retracted a related claim of "
         "their own: their earlier 'my suite was replaced' was an `ls | head` "
         "TRUNCATION, not a deletion — a FILTERED VIEW MISTAKEN FOR THE WHOLE SET, "
         "which is the same failure as trusting a green-looking tests/ directory that "
         "is actually 52 collected tests and an abort. Two instances of one shape in "
         "one repo in one evening."),
 ("good", "PAYER-CANONICAL p95 MEASURED AND COMMITTED, 2026-09-09: 25 sequential "
          "strict-mode calls against prod — p50 188ms, p95 251ms, max 845ms "
          "(first-call warm-up), 0 errors. The payor seat commits to p95 <= 500ms "
          "WARM and explicitly scopes the cold tail out of that number: one earlier "
          "probe hit a 30s+ read timeout on what looked like a cold revision spin-up. "
          "That is the right split — the cold-start tail is the min-instances "
          "discussion, not the query's — and it is a second independent arrival at the "
          "same conclusion the fleet seat and I reached on mobius-payor's own min:1 "
          "quality floor. Appeals' non-blocking resolver_unavailable posture stays "
          "right regardless of the p95, which is the part worth keeping: a measured "
          "latency commitment does not remove the need for the caller to survive its "
          "absence."),
 ("watch", "PATTERN, appeals seat 2026-09-09 — AN ARTEFACT WHOSE IDENTITY IS WRONG "
           "RATHER THAN WHOSE PATH IS MISSING. Distinct from the three "
           "artefact-without-a-path instances collected tonight, and worse in one "
           "respect: a template stored in the payor column READS AS DATA EVERYWHERE IT "
           "IS CONSUMED. It gets counted in coverage, quoted in status, and generates "
           "work for other teams — it nearly bought a Lexicon key from me and an "
           "identity ruling from the payor seat for something that is not a payor. "
           "Nothing is broken and nothing is missing; the row is simply not the kind of "
           ...
           "uniform."),
 ("watch", "PATTERN — THE INSTRUCTION FORBIDS IT AND THE WORKED EXAMPLE SUPPLIES THE MATERIAL. Appeals seat, 2026-09-10, third instance of the fabricated-deadline sweep and a DIFFERENT SPECIES from the first two. api/prompts.py carries RULE 5: \"ONLY REAL VALUES. Every number, form name, URL, fax, portal and rung name you write must appear verbatim in PAYOR FACTS. Never carry a number over from another payor, never round, never infer \'typically 90 days\'. A fabricated deadline is the worst possible output.\" FORTY LINES LATER, worked examples labelled \"this is the bar — match this register exactly\" contain: \"Sunshine Health\'s documented filing limit is 180 days from DOS (90 days for appeals) — per the Provider Manual.\" So the instruction forbids fabrication and THE DEMONSTRATION HANDS OVER CONCRETE NUMBERS PLUS A SOURCE ATTRIBUTION READY TO COPY. A model told to match a register reproduces its CONTENT when its own facts block is empty — which is the state of 100% of that corpus (0 of 339 values sourced). And they are Sunshine\'s real numbers, so carrying them is exactly the \"never carry a number over from another payor\" the example makes easy.\n\nTESTED AND NOT LEAKED, with bounds stated by them rather than by me: 51 generated guidance statements across CARCs 29, 22, 50, 197, 16 and 4 — ZERO day-values asserted. RULE 5 held. They filed it as a LATENT structural risk and said plainly it is 51 of roughly 846 statements, six CARCs, one sample: evidence, not proof. They chose to hand over a clean negative with its limits rather than a third scalp.\n\nWHY IT IS A PATTERN AND NOT AN INSTANCE: this is the SAME SHAPE as the tool_manifest collision. appeals_lookup_rules\' description forbids nothing but its EXAMPLE QUERIES captured the playbook tool\'s use case verbatim, and BM25 then ranked the wrong tool first by 4.6x. In both cases the prose\'s RULE was fine and its EXEMPLAR did the damage. THE TELL: the example is more SPECIFIC than the rule, and specificity is what gets copied. Applies to every prompt and every tool description in the fleet.\n\nTHEY ARE FIXING IT ANYWAY — replacing the concrete numbers with a slot marker so the register survives and the material does not — on the grounds that the structure is one prompt edit away from failing and the thing holding it is a rule the model CHOOSES to follow rather than something enforced. That reasoning is the right one.\n\nFOURTH, same family, REPORTED not verified by me: app.py:2582 in their template scaffold asserts submission: \"certified_mail\" on the Provider Claim Dispute rung with no source. The DEADLINE was nulled during P0-0; the CHANNEL was not. Lower stakes, same class."),
 ("good", "MY CLOSING QUESTION ANSWERED, AND THE GUARDED ENDPOINT IS CLEAN. Appeals verified /playbook-guarded has no synthesised not-found path, two ways: code (\"pb = await get_playbook(...); if not pb: return {}\") and LIVE — GET /playbook-guarded/Sunshine%20Health/24?audience=provider returns HTTP 200 and TWO BYTES. Nothing to strip. Which sharpens the earlier finding on a second axis: the guarded route was already correct on a miss while the MCP route was inventing a deadline, so the two doors differed in what they FABRICATE as well as in what they FILTER."),
 ...
 ("bad", "OWNER(appeals-agent, self-reported): THE GUARD IS FAIL-OPEN ON SCALARS AND "
         ...
         "recorded here because the manner of discovery is the point: a contract "
         "description would have CONCEALED it, since visible_to()'s docstring says 'an "
         "undeclared audience serves nothing' and the code does not. Measured: with "
         "audience omitted, invalid, or 'member', appeal_levels and docs_required are "
         "emptied and the _suppressed/_party_suppressed marker is set — but EVERY "
         "SCALAR PASSES THROUGH UNTOUCHED: deadline 90, provider appeals fax "
         "1-833-504-0580, mailing address, portal URL. Their own stated insight was "
         "'PARTY scopes ACTIONS, AUDIENCE scopes VALUES'; they implemented the actions "
         "half and not the values half. So a member-declared consumer receives the "
         "PROVIDER deadline and the PROVIDER appeals fax — the 141-row party-leak "
         "incident in mirror image, one dimension over. Theirs to fix, flagged before "
         "deploying rather than after. CONSEQUENCE FOR CHAT, and it inverts my "
         "previous instruction: DO NOT treat _suppressed / _party_suppressed as "
         "'empty' — the object still carries populated scalars. And audience is NOT "
         "optional: omitting it does not fail closed today."),
 ("bad", "OWNER(appeals-agent): A MISS IS 200-WITH-{} AND 'PAYOR UNKNOWN' IS "
         "BYTE-IDENTICAL TO 'CARC NOT COVERED'. Measured: CARC 24 on a known payor "
         "returns 200 / 2 bytes / {}; a nonexistent payor (Aetna Better Health) "
         "returns 200 / 2 bytes / {}. Never a 404. So chat cannot tell 'we do not "
         "cover that denial code' from 'we have never heard of that plan' — and those "
         "warrant DIFFERENT answers to a user. Appeals has offered the contract change; "
         "it is worth taking, because it is also the cheapest way to detect the "
         "payor-name mismatch above."),
 ("good", "THE KEY-MISMATCH HYPOTHESIS IS DISPROVED WITH EVIDENCE. I proposed that "
          "appeals' real field names might differ from the two chat reads, which would "
          "have explained everything cheaply. Appeals tested it: deadline_appeal_days "
          "and submission_method are top-level, exactly those names, exactly those "
          "types — not nested, not renamed. Recorded because a disproved hypothesis "
          "retired on evidence is worth as much as a confirmed one, and this one was "
          "MINE."),
 ("bad", "OWNER(chat): THE DISCRIMINATOR CHAT NEEDS IS ALREADY ON THE WIRE AND CHAT "
         "READS NEITHER FIELD. Verified firsthand 2026-09-09: '_party_suppressed' and "
         "'_suppressed' have ZERO occurrences anywhere in mobius-chat/app/. Appeals' "
         "/playbook-guarded returns three DISTINGUISHABLE shapes — a lookup miss is {} "
         "with no keys at all; an audience-filtered playbook is the full dict with "
         "appeal_levels: [] and docs_required: [] PLUS '_party_suppressed': "
         "{'appeal_levels': N, 'docs_required': M} carrying the count of what was "
         "withheld; an undeclared audience is the full dict emptied PLUS '_suppressed': "
         "'undeclared or invalid audience — serving nothing'. So the fork Stage 0 could "
         "not resolve was never actually ambiguous ON THE WIRE — chat just never read "
         "the field that resolves it. A producer with no consumer, and for once the "
         "producer is another team's and it works. ALSO A LATENT TRAP: the route's "
         "audience param DEFAULTS TO '', which is not a valid audience, so it serves "
         "nothing with _suppressed set. react_loop.py:3170 and :3176 both pass "
         "audience='provider' explicitly today and are correct — verified. But if that "
         "argument is ever dropped in a refactor, chat receives a SUPPRESSED envelope "
         "rather than a miss, and without reading _suppressed the two are identical. "
         "Needs a test that fails if the argument disappears."),
 ("good", "TURN 143309c1 IS RESOLVED, AND CHAT WAS RIGHT. Appeals pulled all 141 live "
          "playbook rows covering 72 distinct CARC codes; 24 IS NOT AMONG THEM, zero "
          "rows. So 'our resources don't have specific instructions for a CARC 24 "
          "denial' was ACCURATE. The only defect was that chat's span could not prove "
          "it — a pure reporting defect, no appeals-side data problem, and no "
          "instrumentation archaeology needed on this turn. Ananth's hypothesis that "
          "the tool was 'sending something off' is what led here. Appeals also noted "
          "CARC 24 (charges covered under a capitation agreement) is a real archetype "
          "they have not built — capitation denials are usually a ROUTING error, not an "
          "appeal — which is a coverage gap on their side, unrelated to this defect."),
 ("good", "THE 2026-08-07 CONTENT-EMPTY FINDING IS STALE AND MUST NOT BE TUNED "
          "AGAINST. Appeals measured all 141 rows: no-deadline-AND-no-method (chat's "
          "`usable` predicate) is 0 ROWS; no submission_method is 0 rows; "
          "no deadline_appeal_days is 72 rows — 51% of the corpus, ALL FL Medicaid, and "
          "DELIBERATELY BLANK. They removed unsourced hardcoded deadlines (a "
          "fair-hearing 90-day, a provider-dispute 120-day) during P0-0 rather than "
          "leave fabricated numbers in a filing-critical field; Ananth's own summary of "
          "that state was 'FL Medicaid playbooks are blank rather than wrong'. Evidence "
          "audit: 339 filing-critical values, 0 citable, 0.0% sourced — blank is the "
          "fail-closed CORRECT state until Payor Platform's sourcing lands (first "
          "citable value produced 2026-09-08: Sunshine, 90 days, CMS-PRO-PE-Manual "
          "p.50). So chat's `usable` guard would never fire on today's data, and the "
          "dated finding PREDATES the remediation — re-measure before tuning any retry "
          "guard against it. Appeals' /admin/facts/evidence-audit answers 'was there "
          "anything to lose?' at any moment."),
 ("bad", "OWNER(chat): A FOURTH LAYER STAGE 0 DID NOT NAME — THE TOOL RETURNS A "
         "WELL-FORMED ENVELOPE WRAPPING NOTHING, AND IT HAS HAPPENED BEFORE IN THIS "
         "EXACT TOOL. Ananth's hypothesis, 2026-09-09: 'could it be the tool that was "
         "called was sending something off'. Verified at react_loop.py:3153-3206, and "
         "there is a DATED PRECEDENT recorded in the code by Ananth himself "
         "(2026-08-07): appeals_get_playbook 'used to report success=True + "
         "signal=None whenever the HTTP call succeeded, even when the fetched playbook "
         "had neither a deadline nor a submission method — indistinguishable from a "
         "real hit... 7 consecutive appeals_get_playbook calls, all found but "
         "content-empty (\"?d deadline · \"), burned rounds 2-9 of a 10-round budget "
         "with zero loop-detection.' So this failure mode is not hypothetical for this "
         "tool; it is a recurrence class.\n\nTHE STRUCTURE THAT MAKES IT POSSIBLE: "
         "the handler builds result_data = {'found': found, **pb}, and on the "
         "not-found path pb is a SYNTHESISED FALLBACK — {'message': 'No playbook for "
         "<payor>. Default FL Medicaid: 60 days, certified mail.'} with found=False. "
         "So a call that found nothing still returns a plausible, well-formed, "
         "non-empty payload. TWO DIFFERENT THINGS BOTH LOOK LIKE 'success': a real "
         "playbook, and a manufactured default announcing its own absence.\n\nWHICH "
         "MEANS TURN 143309c1 MAY NOT BE A BUG AT ALL. If it took the not-found path, "
         "the model read found=False plus a message saying no playbook exists and "
         "CORRECTLY said 'our resources don't have specific instructions for a CARC 24 "
         "denial' — a right answer, mislabelled by a span that recorded 'success'. The "
         "defect would then be entirely in the REPORTING, not the behaviour. Chat "
         "Master was right that it is unresolvable on current evidence; this names the "
         "specific fork the evidence cannot resolve, which is more actionable than "
         "'L3 or correct'. RESOLVE IT BY: persisting `usable` (already computed at "
         ":3202 as found and (days is not None or bool(method))) and the found flag "
         "beside the success flag, then re-reading 143309c1. The handler ALREADY "
         "distinguishes all three states in its emit strings — usable, found-but-empty, "
         "not-found — and throws that distinction away before anything durable records "
         "it."),
 ("bad", "OWNER(chat): THE ONLY LIVE CONSUMER OF THE TOOL-RESULT STORE IS THE ONE "
         "THAT EMPTIES IT. chat_tool_results holds 0 rows in the entire dev database. "
         "save_tool_result (writer): 0 external callers. get_tool_result (reader): 0. "
         "format_cached_result: 0. clear_tool_results (DELETE): 1 — state_load.py:130, "
         "on every STANDALONE turn. Written by nothing, read by nothing, cleared "
         "faithfully. This is the purest instance of the producer-without-a-consumer "
         "class the program has found: not a producer whose output nobody reads, but a "
         "STORE WHOSE ONLY WORKING PART IS ITS DELETE PATH. Found by Chat Master in "
         "P3 Stage 0, 2026-09-09. It sits exactly between the two layers Stage 0 was "
         "asked to separate, which is why the traced instance could not be resolved."),
 ("bad", "OWNER(chat): STAGE 0 COULD NOT NAME THE LAYER, AND THE REASON IS THE "
         "FINDING — TWO OF THE FOUR FUNNEL STAGES HAVE NO INSTRUMENT. Measured over "
         "78 turns / 10.75h on dev. (1) OFFERED is recorded as '__unfiltered__' on "
         "all 167 records — that records the FILTER, not the MANIFEST. allowed_tools "
         "was None on 76 of 76 turns, so subscription/policy narrowing IS eliminated; "
         "but None means 'all MODE-APPROPRIATE tools', so mode still shapes the "
         "contents and nothing records them. We can say no tool was filtered out. We "
         "cannot say any tool was OFFERED. (2) CALLED is '__none__' for both 'chose "
         "nothing' and 'emitted unparseable JSON' — the parser returns None or logs at "
         "DEBUG and both land on the same value. Ananth's json-formatting hypothesis "
         "is therefore UNTESTABLE as instrumented; its one logger.warning fired 0 "
         "times in 12h but covers only the late balanced-extract path, so zero "
         "warnings is NOT evidence of zero parse failures and must not be reported as "
         "one. (3) RETURNED-NONEMPTY has no instrument at all: success is "
         "result.get('success') and does not imply content. (4) CLAIMED-ABSENCE has no "
 ...
 ("good", "THE EMIT->DISPATCH BOUNDARY IS CLEAN AND IS NOW ELIMINATED AS A CAUSE: 86 "
          ...
          "allowed_tools=None on 76 of 76 turns, Stage 0 eliminated two candidate "
          "causes on persisted evidence rather than by argument — which is the whole "
          "point of diagnosing before fixing."),
