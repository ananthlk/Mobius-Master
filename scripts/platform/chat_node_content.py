...
""", findings=[
 ...
 ("bad", "OWNER(appeals-agent, ruling requested): CHAT INVENTS AN APPEAL DEADLINE "
         "WHEN THE APPEALS SERVICE FINDS NOTHING, AND NOBODY WHO OWNS THE DOMAIN "
         "APPROVED THE NUMBER. On the not-found path react_loop.py:3178/3181 "
         "synthesises pb = {'message': 'No playbook for <payor>. Default FL Medicaid: "
         "60 days, certified mail.'}. Two problems, and the second is the serious one. "
         "(1) It makes a miss indistinguishable from a hit downstream — a "
         "well-formed non-empty payload either way, which is why turn 143309c1 cannot "
         "be adjudicated. (2) A WRONG APPEAL DEADLINE IS A REAL-WORLD HARM: a provider "
         "who files on day 55 against a plan whose real window is 30 days has lost the "
         "appeal. Chat is generating a specific, actionable, jurisdiction-flavoured "
         "number ('60 days, certified mail') for payors it has no playbook for, and "
         "the number was chosen in chat's own code rather than by the appeals domain. "
         "Raised to the appeals agent 2026-09-09 for a RULING, not a fix: should a "
         "not-found return nothing and let the answer say so? Filed as theirs because "
         "the judgement is domain, not plumbing; the change, if ruled, is chat's."),
